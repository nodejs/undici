...
* **reset** `boolean` (optional) - Default: `true` if `method` is `HEAD` - Whether the connection should be reset after request has completed. This can be necessary when the server misbehaves and doesn't properly terminate messages during socket re-use.
