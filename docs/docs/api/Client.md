...
* **bodyIdleTimeout** `number | null` (optional) - Default: `0` (disabled) - A socket-read inactivity timeout for the response body, in milliseconds (HTTP/1.1). Unlike `bodyTimeout`, it is refreshed only when bytes are read from the socket and fires even while the response body consumer is applying backpressure. This detects a silent or half-open upstream that stops sending mid-response while the body is consumed slowly — a case `bodyTimeout` deliberately ignores (a paused parser means the consumer asked undici to stop reading). Use `0` to disable it entirely.
