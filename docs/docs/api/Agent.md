...
* **maxOrigins** `number` (optional) - Default: `Infinity` - Limits the total number of origins that can receive requests at a time, throwing an `AgentMaxOriginsReached` error when attempting to dispatching when the max is reached. If `Infinity`, no limit is enforced.
