...
const { kReadyState, kSentClose } = require('./symbols')
const { isValidStatusCode, failWebsocketConnection } = require('./util')
  ...
  run (callback) {
    if (this.#state === parserStates.INFO) {
      ...
      if (fragmented && this.#info.opcode !== opcodes.BINARY && this.#info.opcode !== opcodes.TEXT) {
        ...
        failWebsocketConnection(this.ws)
        return
      }
      ...
      if (
        ...
        const body = buffer.subarray(2, payloadLength)

        this.#info.closeInfo = this.parseCloseBody(true, body)
