...
function receiveData (ws) {
  ...
  response.socket.on('data', (chunk) => {
    ...
    if (!frame) {
      ...
      if (receivedFrame.fin && opcode === opcodes.CONTINUATION) {
    ...
    if (frame.opcode === opcodes.PING) {
      ...
      if (!ws[kSentClose]) {
        const { code } = frame.parseCloseBody(ws[kReadyState], true)
        ...
        response.socket.write(closeFrame.createFrame(opcodes.CLOSE))
        ws[kSentClose] = true
      }
...
function handlePing (socket, ws) {
  ...
  const sendFrame = new WebsocketFrameSend()
