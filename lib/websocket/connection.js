...
function receiveData (ws) {
  ...
  const partialFrame = { chunks: [], length: 0, payloadLength: undefined }

  response.socket.on('data', (chunk) => {
    if (!frame) {
      partialFrame.chunks.push(chunk)
      partialFrame.length += chunk.length

      if (partialFrame.length < 2) {
        return // Can't read payload length yet
      } else if (partialFrame.payloadLength !== undefined) {
        if (partialFrame.payloadLength === 126) {
          if (partialFrame.length < 4) {
            return
          }
        } else if (partialFrame.payloadLength === 127) {
          if (partialFrame.length < 10) {
            return
          }
        }
      } else {
        const data = Buffer.concat(partialFrame.chunks, partialFrame.length)
        const payloadLength = data[1] & 0x7F

        partialFrame.chunks = [data]
        partialFrame.payloadLength = payloadLength
    ...
    if (partialFrame.payloadLength !== undefined) {
      receivedFrame = WebsocketFrame.from(Buffer.concat(partialFrame.chunks, partialFrame.length))
      partialFrame.chunks.length = 0
      partialFrame.length = 0
      partialFrame.payloadLength = undefined
    } else {
      receivedFrame = WebsocketFrame.from(chunk)
    }
