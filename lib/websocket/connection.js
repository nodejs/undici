 ...
 */
  ...
  toBuffer () {
    ...
    const payloadLength = this.payloadData?.length ?? 0
    const buffer = Buffer.alloc(4 + 2 + payloadLength)
...
function socketClosed (ws) {
  ...
  response.socket.on('close', () => {
    ...
    let reason = buffer.payloadData.toString('utf-8', 4)

    // https://datatracker.ietf.org/doc/html/rfc6455#section-7.1.5
    let code = ws[kReadyState] === states.CLOSED ? 1006 : 1005

    if (buffer.payloadLength >= 2) {
      code = buffer.payloadData.readUInt16BE(2)
    }
