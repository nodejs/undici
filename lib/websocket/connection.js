...
async function obtainWebSocketConnection (url, agent) {
  ...
  //    (/).
  let { origin: host, port, pathname: resourceName } = url
  ...
  const socket = await rfc6455OpeningHandshake(host, port, resourceName, secure, agent)
 ...
 * @param {import('../..').Agent} agent
 */
function rfc6455OpeningHandshake (host, port, resourceName, secure, agent) {
