...
function receiveData (ws) {
  ...
  response.socket.on('data', (chunk) => {
    if (!frame) {
      frame = WebsocketFrame.from(chunk)
      frame.terminated = frame.fin // message complete
    } else {
      ...
      const fragmentedFrame = WebsocketFrame.from(chunk)

      if (fragmentedFrame.fin && fragmentedFrame.opcode === opcodes.CONTINUATION) {
        frame.terminated = true
      }

      frame.addFrame(fragmentedFrame.data)
    }
    ...
    if (frame.opcode === opcodes.PING) {
      ...
      const buffer = frame.createFrame(opcodes.PONG)
    ...
    assert(frame.opcode === opcodes.TEXT || frame.opcode === opcodes.BINARY)
...
function socketClosed (ws) {
  ...
  response.socket.on('close', () => {
    ...
    const { code, reason } = ws[kClosingFrame]
      ? ws[kClosingFrame].parseCloseBody(ws[kReadyState])
      : { code: 1005, reason: '' }
