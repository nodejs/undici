...
class RedirectHandler {
  constructor ({ agent, opts, origin, handler }) {
    this.agent = agent
    this.location = null
    this.abort = null
    this.opts = opts
  ...
  onHeaders (statusCode, headers, resume) {
    ...
    const { origin, pathname, search } = util.parseURL(new URL(this.location, this.origin))
...
class RedirectAgent extends EventEmitter {
  constructor ({ maxRedirections = 10, agent = getGlobalAgent() } = {}) {
    super()
    ...
    this[kAgent].on('connect', (...args) => this.emit(...args))
    this[kAgent].on('disconnect', (...args) => this.emit(...args))
  }
  ...
  dispatch ({ origin, maxRedirections = this[kMaxRedirections], ...opts } = {}, handler) {
    if (maxRedirections != null && (maxRedirections < 0 || !Number.isInteger(maxRedirections))) {
      throw new InvalidArgumentError('maxRedirections must be a positive number')
    }

    if (!maxRedirections) {
      this[kAgent].dispatch(opts, handler)
      return
    }
    ...
    const redirectHandler = new RedirectHandler({ agent: this, opts, origin, handler })
    ...
    this[kAgent].dispatch(opts, redirectHandler)
  }
