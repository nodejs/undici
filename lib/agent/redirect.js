...
class RedirectHandler {
  ...
  onHeaders (statusCode, headers, resume) {
    ...
    const path = search ? `${pathname /* istanbul ignore next */ || '/'}${search}` : pathname
  ...
  onData (chunk) {
    if (this.location) {
      /*
        ...
        no body is generated (and thus can be ignored). Even thought generating a body is not prohibited.
  ...
  onComplete (trailers) {
    if (this.location) {
      /*
        ...
        TLDR: undici always ignores 3xx response trailers.

        See onData method comment for more informations.
      */
