...
class Agent extends EventEmitter {
  ...
  close () {
    ...
    return Promise.all(closePromises).then((closedPromises) => {
      return new Promise((resolve) => {
        this[kClosed] = true
        resolve(closedPromises)
      })
    })
  }
  ...
  destroy () {
    ...
    return Promise.all(destroyPromises).then((destroyedPromises) => {
      return new Promise((resolve) => {
        this[kDestroyed] = true
        resolve(destroyedPromises)
      })
    })
  }
}
