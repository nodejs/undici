...
const { InvalidArgumentError, InformationalError, DNSFilterError } = require('../core/errors')
...
class DNSInstance {
  ...
  filterRecords (origin, records) {
    ...
    return Array.from(records).filter(record => this.filter(origin, record) === true)
  }
  ...
  return dispatch => {
    return function dnsInterceptor (origDispatchOpts, handler) {
      ...
      if (family !== 0) {
        ...
        if (records.length === 0) {
          return handler.onResponseError(null, new DNSFilterError())
        }
