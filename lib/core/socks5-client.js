...
class Socks5Client extends EventEmitter {
  ...
  buildConnectRequest (command, address, port) {
    // Determine address type and prepare address buffer
    let addressType
    let addressBuffer
    ...
    if (ipv4Match) {
      addressType = ADDRESS_TYPES.IPV4
      addressBuffer = Buffer.from(ipv4Match.slice(1, 5).map(Number))
    } else if (address.includes(':')) {
      // IPv6 address
      addressType = ADDRESS_TYPES.IPV6
      // Parse IPv6 address
      address.split(':')
      addressBuffer = Buffer.alloc(16)
      // TODO: Proper IPv6 parsing
      throw new InvalidArgumentError('IPv6 not yet implemented')
    } else {
      // Domain name
      addressType = ADDRESS_TYPES.DOMAIN
      const domainBuffer = Buffer.from(address)
      if (domainBuffer.length > 255) {
        throw new InvalidArgumentError('Domain name too long')
      }
      addressBuffer = Buffer.concat([Buffer.from([domainBuffer.length]), domainBuffer])
    }
