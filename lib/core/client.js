...
class Client extends EventEmitter {
  constructor (url, {
    ...
    if (tls && tls.maxCachedSessions != null && (!Number.isFinite(tls.maxCachedSessions) || tls.maxCachedSessions < 0)) {
      throw new InvalidArgumentError('invalid tlsMaxCachedSessions')
    }
    ...
    this[kTLSMaxCachedSessions] = (tls == null || tls.maxCachedSessions == null) ? 100 : tls.maxCachedSessions
    if (tls && tls.maxCachedSessions) delete tls.maxCachedSessions
    this[kTLSOpts] = tls
    this[kRetryDelay] = 0
...
function connect (client) {
  ...
  if (protocol === 'https:') {
    ...
    socket.on('session', (newSession) => {
      if (client[kTLSSessions].size >= client[kTLSMaxCachedSessions]) {
...
function createName (url, tls) {
  ...
  if (tls) {
    ...
    name += ':'
    if (tls.clientCertEngine) {
      name += tls.clientCertEngine
    }
    ...
    name += ':'
    if (tls.servername && tls.servername !== tls.host) {
      name += tls.servername
    }
    ...
    name += ':'
    if (tls.secureProtocol) {
      name += tls.secureProtocol
    }
    ...
    name += ':'
    if (tls.honorCipherOrder !== undefined) {
      name += tls.honorCipherOrder
    }
    ...
    name += ':'
    if (tls.secureOptions !== undefined) {
      name += tls.secureOptions
    }

    name += ':'
    if (tls.sessionIdContext) {
      name += tls.sessionIdContext
    }
  }
  return name
}
