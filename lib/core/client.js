...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnHeadersComplete](
    ...
    if (shouldKeepAlive && client[kPipelining]) {
      const keepAliveTimeout = keepAlive
        ? util.parseKeepAliveTimeout(keepAlive)
        : null
      ...
      if (trailers) {
        ...
        for (const trailer of trailers) {
          ...
          for (let n = 0; n < headers.length; n += 2) {
            const key = headers[n + 0]
            if (
              key.length === trailer.length &&
              key.toLowerCase() === trailer.toLowerCase()
            ) {
              found = true
              break
            }
...
function onSocketError(err) {
  ...
  if (err.code === 'ERR_TLS_CERT_ALTNAME_INVALID') {
    ...
    while (
      client.pending &&
      client[kQueue][client[kPendingIdx]].host === client[kHost]
    ) {
  ...
  while (true) {
    ...
    if (client[kSocket]) {
      ...
      if (socket[kIdleTimeoutValue] !== timeout) {
        clearTimeout(socket[kIdleTimeout])
        if (timeout) {
          socket[kIdleTimeout] = setTimeout(
            (socket) => {
              util.destroy(
                socket,
                new InformationalError('socket idle timeout')
              )
            },
            timeout,
            socket
...
function write(client, request) {
  ...
  if (!body) {
    ...
    assert(
      contentLength !== 0 || !client.running,
