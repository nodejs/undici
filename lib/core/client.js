...
class Client extends EventEmitter {
  constructor (url, {
    ...
    this.tlsSession = tls && tls.session ? tls.session : null
  ...
  set tlsSession (session) {
    this[kTLSSession] = session
    if (session) {
      console.log(`client ${this.id} received new session`, session.toString('hex').substr(-5))
    } else {
      console.log(`client ${this.id} received no session`)
    }
...
function connect (client) {
  ...
  if (protocol === 'https:') {
    const tlsOpts = {
      ...
      session: client.tlsSession
    }
    if (client.tlsSession) {
      console.log(`client ${client.id} using existing session`, client.tlsSession)
    }
