...
function connect (client) {
  ...
  if (protocol === 'https:') {
    ...
    const sessionName = createName(client[kUrl], tlsOpts)
    // Get cached session if it does exists
    const session = client[kTLSSessions].get(sessionName)
    if (session !== undefined) tlsOpts.session = session
    ...
    socket.on('session', (newSession) => {
      if (client[kTLSSessions].size >= 100) {
        // `map.keys()` returns an iterator of the keys in insertion order
        const firstInsertedKey = client[kTLSSessions].keys().next().value
        client[kTLSSessions].delete(firstInsertedKey)
      }
      client[kTLSSessions].set(sessionName, newSession)
    })

    // Evict session on error
    socket.once('close', (err) => {
      if (err) {
        client[kTLSSessions].delete(sessionName)
      }
    })
  } else {
    socket = client[kSocketPath]
...
function createName (url, tls) {
  let name = `${url.hostname}:${url.port}`
  if (tls) {
    name += ':'
    if (tls.ca) { name += tls.ca }

    name += ':'
    if (tls.cert) { name += tls.cert }

    name += ':'
    if (tls.clientCertEngine) { name += tls.clientCertEngine }

    name += ':'
    if (tls.ciphers) { name += tls.ciphers }

    name += ':'
    if (tls.key) { name += tls.key }

    name += ':'
    if (tls.pfx) { name += tls.pfx }

    name += ':'
    if (tls.rejectUnauthorized !== undefined) { name += tls.rejectUnauthorized }

    name += ':'
    if (tls.servername && tls.servername !== tls.host) { name += tls.servername }

    name += ':'
    if (tls.minVersion) { name += tls.minVersion }

    name += ':'
    if (tls.maxVersion) { name += tls.maxVersion }

    name += ':'
    if (tls.secureProtocol) { name += tls.secureProtocol }

    name += ':'
    if (tls.crl) { name += tls.crl }

    name += ':'
    if (tls.honorCipherOrder !== undefined) { name += tls.honorCipherOrder }

    name += ':'
    if (tls.ecdhCurve) { name += tls.ecdhCurve }

    name += ':'
    if (tls.dhparam) { name += tls.dhparam }

    name += ':'
    if (tls.secureOptions !== undefined) { name += tls.secureOptions }

    name += ':'
    if (tls.sessionIdContext) { name += tls.sessionIdContext }
  }
  return name
}
