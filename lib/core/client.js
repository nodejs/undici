...
function write (client, request) {
  ...
  if (!body) {
    ...
    const stringOrBuffer = x => {
      if (typeof x === 'string') {
        return 1
      } else if (util.isBuffer(x)) {
        return 0
      } else {
        throw new Error('Body iterable must yield either string or buffer')
      }
    }
    let chunks = stringOrBuffer(result.value) ? result.value : [result.value]
    while (true) {
      if (result.done) break
      result = iterator.next()
      if (stringOrBuffer(result.value)) {
        chunks += result.value
      } else {
    ...
    socket.cork()
    socket.write(`${header}content-length: ${chunks.length}\r\n\r\n`, 'ascii')
    socket.write(typeof chunks === 'string' ? chunks : Buffer.concat(chunks))
    socket.write('\r\n', 'ascii')
    socket.uncork()
