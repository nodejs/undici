...
function _resume (client, sync) {
  while (true) {
    ...
    if (client.running &&
      !util.isBuffer(request.body) &&
      typeof request.body !== 'string' &&
      (util.isStream(request.body) || util.isIterable(request.body))) {
...
async function writeIterable (socket, contentLength, header, body) {
  try {
    ...
    for await (const chunk of body) {
      // if (typeof chunk !== 'string' && !util.sBuffer(chunk)) {
      //   throw new Error('Body iterable must yield either string or buffer')
      // }
...
function write (client, request) {
  ...
  if (!body) {
    ...
    writeIterable(socket, contentLength, header, body)
    request.body = null
  }
