...
class Client extends EventEmitter {
  constructor (url, {
    ...
    this[kConnector] = connector || new Connector({ url, socketPath, tls })
    this[kSocket] = null
    this[kReset] = false
...
function connect (client) {
  ...
  const servername = (
    client[kHost] &&
    !/^\[/.test(client[kHost]) &&
    !net.isIP(client[kHost])
  ) ? client[kHost] : undefined

  const socket = client[kConnector].connect({ servername }, onSocketConnect)
...
function _resume (client, sync) {
  while (true) {
    ...
    if (client[kHost] !== request.host) {
      client[kHost] = request.host
      if (client[kSocket] && !client.running) {
...
function write (client, request) {
  ...
  if (!host) {
    header += `host: ${client[kConnector].host}\r\n`
  }
