...
class Client extends EventEmitter {
  constructor (url, {
    ...
    this[kConnector] = connector || new Connector({ socketPath, tls })
    this[kSocket] = null
    this[kReset] = false
...
function connect (client) {
  ...
  const socket = client[kConnector].connect({
    protocol: client[kUrl].protocol,
    port: client[kUrl].port,
    hostname: client[kUrl].hostname
  }, onSocketConnect)
...
function _resume (client, sync) {
  while (true) {
    ...
    // else if (client[kConnector].reconnect({ ...client[kUrl], ...request })) {
    //   if (!client.running) {
