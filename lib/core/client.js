...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnHeadersComplete] (versionMajor, versionMinor, rawHeaders, method,
    ...
    if (statusCode >= 200 && request.bodyTimeout) {
      this.bodyTimeout = setTimeout((self) => {
        if (!self.paused) {
          util.destroy(self.socket, new BodyTimeoutError())
        }
      }, request.bodyTimeout, this)
    }
  ...
  [HTTPParser.kOnBody] (chunk, offset, length) {
    ...
    const { socket, statusCode, request, bodyTimeout } = this
  ...
  [HTTPParser.kOnMessageComplete] () {
    ...
    const { client, socket, statusCode, headers, upgrade, request, trailers, bodyTimeout } = this
