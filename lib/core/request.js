...
class Request {
   ...
   * @param {string[]} [keys] lowercased name of each field line, from the parser
   */
  onResponseStart (statusCode, headers, resume, statusText, keys) {
    assert(!this.aborted)
    assert(!this.completed)
    ...
    const parsedHeaders = Array.isArray(headers) ? parseHeadersWithKeys(headers, undefined, keys) : headers
   ...
   * @param {string[]} [keys] lowercased name of each field line, from the parser
   */
  onRequestUpgrade (statusCode, headers, socket, statusText = '', keys) {
    this.onFinally()
  ...
  onResponseEnd (trailers, keys) {
    ...
    const parsedTrailers = Array.isArray(trailers) ? parseHeadersWithKeys(trailers, undefined, keys) : trailers
