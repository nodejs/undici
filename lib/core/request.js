...
class Request {
   ...
   * @param {Uint16Array} [wellknown] well-known name index of each field line, from the parser
   */
  onResponseStart (statusCode, headers, resume, statusText, wellknown) {
    assert(!this.aborted)
    assert(!this.completed)
    ...
    const parsedHeaders = Array.isArray(headers) ? parseHeadersWithWellknown(headers, undefined, wellknown) : headers
   ...
   * @param {Uint16Array} [wellknown] well-known name index of each field line, from the parser
   */
  onRequestUpgrade (statusCode, headers, socket, statusText = '', wellknown) {
    this.onFinally()
  ...
  /**
   * @param {Buffer[]|string[]|import('../../types/header.d.ts').IncomingHttpHeaders} trailers
   * @param {Uint16Array} [wellknown] well-known name index of each field line, from the parser
   */
  onResponseEnd (trailers, wellknown) {
    this.onFinally()
    ...
    const parsedTrailers = Array.isArray(trailers) ? parseHeadersWithWellknown(trailers, undefined, wellknown) : trailers
