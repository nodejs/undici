...
class Request {
  constructor ({
    ...
    if (body == null) {
      ...
      try {
        this.body = typeof body === 'function' ? Readable.from(body()) : Readable.from(body)
      } catch (err) {
        if (err.code === 'ERR_INVALID_ARG_TYPE') {
          throw new InvalidArgumentError('body must be a string, a Buffer, a Readable stream, or an iterable')
        } else {
          throw err
        }
      }
