...
class RequestController {
  ...
  get rawHeadersH2 () {
    return this.#rawHeadersH2
  }

  set rawHeadersH2 (value) {
    this.#rawHeadersH2 = value
    this.#rawHeaders = null
  }

  get rawHeaders () {
    if (this.#rawHeadersH2 !== null) {
      const source = this.#rawHeadersH2
      // Skip the :status pseudo-header. Convert only when a consumer needs
      // the dispatcher raw-header (Buffer[]) representation.
      const result = new Array(source.length - 2)
      for (let i = 2; i < source.length; i++) {
        result[i - 2] = Buffer.from(source[i], 'latin1')
      }
      this.#rawHeaders = result
      this.#rawHeadersH2 = null
    }
...
class Request {
  ...
  onResponseStart (statusCode, headers, resume, statusText, rawHeadersH2) {
    assert(!this.aborted)
    assert(!this.completed)
    ...
    if (controller) {
      ...
      if (rawHeadersH2 != null) {
        controller.rawHeadersH2 = rawHeadersH2
      } else {
        controller.rawHeaders = headers
      }
    }

    const parsedHeaders = rawHeadersH2 != null
      ? parseHeaders(rawHeadersH2, undefined, 2)
      : Array.isArray(headers) ? parseHeaders(headers) : headers
