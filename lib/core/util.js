...
function nodeMajorVersionIsGreaterThanOrEqualTo (expectedMajor) {
  const [foundMajor] = process.versions.node.split('.').map(str => parseInt(str, 10))

  return foundMajor >= expectedMajor
}
...
module.exports = {
  ...
  bodyLength,
  isBuffer,
  nodeMajorVersionIsGreaterThanOrEqualTo,
  weakCache
}
