...
/**
 * @param {Record<string, string | string[]> | Iterable<[string, string]> | string[] | null | undefined} headers
 * @returns {Record<string, string[] | string>}
 */
function normalizeHeaders (headers) {
  ...
  if (typeof src !== 'object' || src === null) return normalized

  if (Array.isArray(src)) {
    for (let i = 0; i < src.length; i += 2) {
      normalized[src[i]] = src[i + 1]
    }
    return normalized
  }

  if (typeof src[Symbol.iterator] === 'function') {
    const errMsg = 'headers is not a valid header map'
    for (const s of src) {
      if (!Array.isArray(s)) throw new InvalidArgumentError(errMsg)
      const [key, val] = s
      if (typeof key !== 'string' || typeof val !== 'string') {
        throw new InvalidArgumentError(errMsg)
      }
      const lowerKey = headerNameLowerCasedRecord[key] ?? key.toLowerCase()
