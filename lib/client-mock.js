...
class MockClientScope {
  ...
  times (repeatTimes) {
    mockDispatches.get(this.key)[this.index].times = repeatTimes
    return this
  }
}
  ...
  intercept (opts) {
    ...
    return {
      reply: (statusCode, data, responseOptions = {}) => {
        ...
        const newDispatchData = { ...baseData, statusCode, data, headers, trailers }
      ...
      replyWithError: (error) => {
        const index = addToMockDispatches(key, { ...baseData, error })
        this.mockDispatchKeys.push([key, index])
        return new MockClientScope(key, index)
      }
      // TODO
