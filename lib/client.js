...
function _connect (client) {
  ...
  client[kStream].finished(socket, (err) => {
    ...
    // reset events
    // TODO: Only remove our listeners.
...
class Client extends EventEmitter {
  ...
  destroy (err, cb) {
    ...
    if (!this.socket) {
      process.nextTick(onDestroyed, null, null)
    } else if (this.socket[kClosed] || this.socket[kError]) {
      process.nextTick(onDestroyed, this.socket[kError], null)
      this.socket[kError] = null
    } else {
      this.socket
        .on('close', () => {
          onDestroyed(this.socket[kError])
        })
        .destroy(err)
    }
  }
