  ...
  [HTTPParser.kOnHeadersComplete] ({ statusCode, headers }) {
    ...
    if (body) {
      if (typeof body.write === 'function') {
        ...
        // TODO: Don't assume 'error' and 'close' will set destroyed to ´true`.
        body
          .once('drain', this.resume)
          .once('close', this.resume)
        if (EventEmitter.errorMonitor) {
          body.once(EventEmitter.errorMonitor, this.resume)
        } else {
          body.once('error', (err) => {
            this.resume()
            if (body.listenerCount('error') === 0) {
              body.emit('error', err)
            }
          })
        }
