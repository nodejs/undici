...
const llhttp = new WebAssembly.Instance(mod, {
  env: {
    ...
    wasm_on_header_field: (p, at, len) => {
      const parser = parsers.get(p)

      const start = at - parser.bufferPtr
      const end = start + len
      return parser.onHeaderField(parser.bufferRef.slice(start, end)) || 0
    },
    wasm_on_header_value: (p, at, len) => {
      ...
      const start = at - parser.bufferPtr
      const end = start + len
      return parser.onHeaderValue(parser.bufferRef.slice(start, end)) || 0
    },
    wasm_on_headers_complete: p => {
      ...
      return parser.onHeadersComplete(statusCode, upgrade, shouldKeepAlive) || 0
    },
    wasm_on_body: (p, at, len) => {
      ...
      const start = at - parser.bufferPtr
      const end = start + len
      return parser.onBody(parser.bufferRef.slice(start, end)) || 0
    },
