...
const currentBufferSize = 16384
const currentBufferPtr = llhttp.exports.malloc(currentBufferSize)
const currentBufferView = new Uint8Array(llhttp.exports.memory.buffer, currentBufferPtr, currentBufferSize)
...
class Parser {
  ...
  execute (data) {
    ...
    assert(currentParser == null)
    assert(!this.paused)
    assert(data.length <= currentBufferView.length)
...
function onSocketData (data) {
  ...
  if (data.length <= currentBufferSize) {
    ...
    for (let n = 0; n < data.length; n += currentBufferSize) {
      parser.execute(data.slice(n, currentBufferSize))
      data = data.slice(currentBufferSize)
    }
  }
}
