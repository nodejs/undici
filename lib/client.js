...
class Client extends EventEmitter {
  constructor (url, {
    ...
    this[kRequestTimeout] = requestTimeout === undefined ? 30e3 : requestTimeout
    this[kClosed] = false
  ...
  request (opts, callback) {
    ...
    opts.requestTimeout = opts.requestTimeout !== undefined
      ? opts.requestTimeout
      : this[kRequestTimeout]
...
function resume (client) {
  if (client[kDestroyed]) {
    for (const request of client[kQueue].splice(client[kInflight])) {
      const { callback } = request
      if (callback) {
        request.reset()
        process.nextTick(callback, new ClientDestroyedError(), null)
