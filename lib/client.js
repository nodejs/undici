...
function _connect (client) {
  ...
  client._parser[HTTPParser.kOnBody] = (chunk, offset, length) => {
    client[kCurrent].read += length

    const { read, body } = client[kCurrent]
  ...
  function onFinished (err) {
    ...
    const { callback, body, reset } = client[kCurrent]
    client[kCurrent] = null
...
class Client extends EventEmitter {
  constructor (url, opts = {}) {
    ...
    this[kQueue] = Q((request, callback) => {
      ...
      this[kInflight].push({ request, callback, reset })
