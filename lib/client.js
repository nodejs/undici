...
class Client extends Dispatcher {
  constructor (url, {
    ...
    this[kSocket] = null
    this[kMaxPipelining] = pipelining != null ? pipelining : 1
    this[kPipelining] = this[kMaxPipelining]
...
class Parser {
  ...
  onHeadersComplete (statusCode, upgrade, shouldKeepAlive) {
    ...
    for (let i = 0; i < headers.length; i+= 2) {
      if (headers[i].toString().toLowerCase() === 'keep-alive') {
        const value = headers[i + 1].toString().toLowerCase();
        const maxIndex = value.indexOf('max=');

        if (maxIndex !== -1) {
          const pipelining = parseInt(value.slice(maxIndex + 4));
          if (Number.isFinite(pipelining)) {
            // What if client[kPipelining] is undefined?
            client[kPipelining] = Math.min(client[kMaxPipelining], pipelining)
          }
        }
      }
    }
