...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnHeadersComplete] (versionMajor, versionMinor, rawHeaders, method,
    ...
    let keepAliveTimeout = Number.isFinite(shouldKeepAlive)
      ? shouldKeepAlive
      : util.parseKeepAliveTimeout(shouldKeepAlive, headers)
    if (keepAliveTimeout >= 0) {
      keepAliveTimeout = Math.min(
        keepAliveTimeout * 1e3 - client[kKeepAliveTimeoutThreshold],
