...
function writeH2 (client, session, request) {
  ...
  const { body, method, path, host, upgrade, expectContinue } = request

  assert(!upgrade, 'Upgrade not supported for H2')
  ...
  if (expectContinue) {
    ...
    stream.once('continue', () => {
      ...
      if (!body) {
        ...
        writeIterable({ body, client, request, socket: client[kSocket], contentLength, expectsPayload })
      } else {
        assert(false)
      }
    })
    ...
    if (!body) {
      ...
      writeIterable({ h2stream: stream, body, client, request, socket: client[kSocket], contentLength, expectsPayload })
    } else {
      assert(false)
    }
  }
