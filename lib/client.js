...
class Parser {
  ...
  onHeaderValue (buf) {
    ...
    if (
      this.headers[len - 2].byteLength === 10 &&
      this.headers[len - 2].toString().toLowerCase() === 'keep-alive'
    ) {
      this.keepAlive += buf.toString()
    } else if (
      this.headers[len - 2].byteLength === 7 &&
      this.headers[len - 2].toString().toLowerCase() === 'trailer'
    ) {
