...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnHeadersComplete] ({ statusCode, headers }) {
    ...
    const { request, callback } = this.inflight.shift()
  ...
  [HTTPParser.kOnMessageComplete] () {
    const { client, socket, body, callback } = this
  ...
  destroy (err) {
    const { client, body, callback } = this
    ...
    const retry = []
    for (const { request, callback } of this.inflight.splice(0)) {
...
class Client extends EventEmitter {
  constructor (url, opts = {}) {
    ...
    this[kQueue] = Q((request, callback) => {
      ...
      this._parser.inflight.push({ request, callback })
