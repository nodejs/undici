...
class Parser extends HTTPParser {
  ...
  onHeadersComplete (rawHeaders, statusCode, shouldKeepAlive) {
    const { client, socket, timeout } = this
  ...
  [HTTPParser.kOnHeadersComplete] (rawHeaders, statusCode, upgrade, shouldKeepAlive) {
  ...
  [HTTPParser.kOnBody] (chunk, at, length) {
    ...
    chunk = Buffer.from(chunk, at, length) // llhttp re-uses buffer so we need to make a copy.
  ...
  [HTTPParser.kOnMessageComplete] (rawTrailers) {
    const { client, socket, statusCode, upgrade, request, trailers, timeout } = this
    ...
    if (this.paused) {
      this.queue.push([HTTPParser.kOnMessageComplete, rawTrailers])
      return
    }
