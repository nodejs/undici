...
class Parser extends HTTPParser {
  ...
  onUpgrade (head) {
    ...
    client.emit('disconnect', { url: client[kUrl], targets: [client], err: new InformationalError('upgrade') })
...
function onSocketConnect () {
  ...
  client.emit('connect', { url: client[kUrl], targets: [client] })
  resume(client)
}
...
function onSocketClose () {
  ...
  if (client[kDestroyed]) {
    ...
    client.emit('disconnect', { url: client[kUrl], targets: [client], err })
  }
...
function emitDrain (client) {
  client[kNeedDrain] = 0
  client.emit('drain', { url: client[kUrl], targets: [client] })
}
