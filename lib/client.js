...
class Client extends EventEmitter {
  constructor (url, {
    ...
    if (abortedTimeout != null && !Number.isFinite(abortedTimeout)) {
      throw new InvalidArgumentError('invalid abortedTimeout')
    }
    ...
    this[kDrained] = false
    this[kAbortedTimeout] = abortedTimeout || 1e3
    this[kResume] = () => {
      resume(this)
    }
...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnMessageComplete] () {
    ...
    if (socket[kAbortedTimeout]) {
      clearTimeout(socket[kAbortedTimeout])
      socket[kAbortedTimeout] = null
    }
...
function onSocketClose () {
  const { 
    [kClient]: client, 
    [kParser]: parser,
    [kAbortedTimeout]: abortedTimeout
  } = this
...
function _resume (client) {
  while (true) {
    ...
    if (client.running) {
      const request = client[kQueue][client[kRunningIdx]]
      if (request.aborted) {
        const running = client[kQueue].slice(client[kRunningIdx], client[kPendingIdx])
        if (client[kClosed] && running.every(x => x.aborted)) {
          socket.destroy()
        } else if (!socket[kAbortedTimeout]) {
          socket[kAbortedTimeout] = setTimeout((socket) => {
            socket.destroy()
          }, client[kAbortedTimeout], socket)
        }
        return
      }      
    }
