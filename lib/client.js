...
class Parser extends HTTPParser {
  ...
  destroy (err) {
    ...
    client[kInflight] = client[kComplete] = 0
...
function resume (client) {
  ...
  if (!host) {
    client._socket.write('Host: ' + client[kUrl].hostname + '\r\n', 'ascii')
  }
  client._socket.write(rawHeaders, 'ascii')

  if (typeof body === 'string' || body instanceof Uint8Array) {
    if (chunked) {
      client._socket.write(`content-length: ${Buffer.byteLength(body)}\r\n\r\n`, 'ascii')
    } else {
      client._socket.write('\r\n')
    }
    client._socket.write(body)
    endRequest(client)
  } else if (body && typeof body.pipe === 'function') {
    if (chunked) {
      client._socket.write('transfer-encoding: chunked\r\n', 'ascii')
    } else {
      client._socket.write('\r\n', 'ascii')
    }
