...
class Client extends EventEmitter {
  constructor (url, {
    ...
    this[kDrained] = false
    this[kAbortedTimeout] = abortedTimeout || 1048576
    this[kResume] = () => {
      resume(this)
    }
...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnBody] (chunk, offset, length) {
    ...
    // TODO: abortedTimeout should count from when aborted
    // not from start.
    ...
    if (ret == null) {
      ...
      if (this.read > client[kAbortedTimeout]) {
        client[kQueue][client[kRunningIdx]++] = null
  ...
  [HTTPParser.kOnMessageComplete] () {
    ...
    if (client[kWriting]) {
      // TOOD: keep writing until abortedTimeout?
    ...
    if (client.running) {
      ...
      if (request.aborted) {
        ...
        if (client[kClosed] && running.every(x => x.aborted)) {
          ...
          client[kSocket].setTimeout(client[kAbortedTimeout])
        }
        return
      }      
    }
