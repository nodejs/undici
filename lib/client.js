...
function _resume (client) {
  while (true) {
    ...
    if (client.running) {
      ...
      if (request.aborted) {
        ...
        if (client[kClosed] && running.every(x => x.aborted)) {
          util.destroy(socket)
        } else if (!socket[kTimeout]) {
          socket[kTimeout] = setTimeout(util.destroy, client[kAbortedTimeout], socket)
        }
        return
      }
    }
    ...
    if (!socket && !client[kRetryTimeout]) {
