...
async function writeIterable (client, request, socket, contentLength, header, expectsPayload) {
  ...
  function onFinished (err) {
    ...
    finished = true
    socket[kWriting] = false
    socket.removeListener('error', onFinished)
    finishChunkedWrite(err, contentLength, bytesWritten, expectsPayload, header, socket, client)
  }

  let thrownError = null
  try {
    socket.on('error', onFinished)
    for await (const chunk of body) {
      if (finished) {
        return
      }
      const chunkSize = writeChunkHeaders(finished, chunk, contentLength, bytesWritten, expectsPayload, header, socket, client)
      if (!chunkSize) {
        continue
      }
      ...
      if (socket.write(chunk)) {
        ...
        await EventEmitter.once(socket, 'drain')
      }
    }
  } catch (err) {
    thrownError = err
  }
  onFinished(thrownError)
}

function writeChunkHeaders (finished, chunk, contentLength, bytesWritten, expectsPayload, header, socket, client) {
  assert(!finished)
...
function finishChunkedWrite (err, contentLength, bytesWritten, expectsPayload, header, socket, client) {
...
function write (client, request) {
  ...
  if (!body) {
    ...
    const onData = function (chunk) {
      try {
        const len = writeChunkHeaders(finished, chunk, contentLength, bytesWritten, expectsPayload, header, socket, client)
        if (!len) {
          return
        }
        bytesWritten += len
    ...
    assert(false, 'body should be empty, buffer, stream or async iterable')
  }
