...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnExecute] (ret) {
    ...
    if (this.upgrade && !this.socket.destroyed) {
      ...
      // _readableState.flowing might be `false` if the socket has been
      // explicitly `pause()`:d. This will stop socket from automatically
      // resuming when 'data' listener is attached.
      ...
      // when 'data' listener is attached.
