...
class Parser {
  ...
  execute (data) {
    ...
    const err = this.inst.llhttp_execute(this.ptr, this.bufferPtr, data.length)
    ...
    if (err === constants.ERROR.PAUSED_UPGRADE) {
      ...
      const len = this.bufferView.indexOf(0, ptr) - ptr
  ...
  onHeadersComplete2 (statusCode, shouldKeepAlive) {
  ...
  onHeadersComplete (statusCode, upgrade, shouldKeepAlive) {
    ...
    if (this.paused) {
      this.queue.push(['onHeadersComplete2', statusCode, shouldKeepAlive])
    } else {
      this.onHeadersComplete2(statusCode, shouldKeepAlive)
    }
  ...
  onMessageComplete () {
    if (this.paused) {
      this.queue.push(['onMessageComplete'])
      return
    }
