  ...
  try {
    ...
    for await (const chunk of body) {
      ...
      if (socket.write(chunk)) {
        continue
      } else if (socket.destroyed || error) {
        return
      } else {
        await waitForDrain()
        if (error) {
    ...
    if (!error && contentLength !== null && bytesWritten !== contentLength) {
      if (client[kStrictContentLength]) {
        error = new RequestContentLengthMismatchError()
      } else {
        process.emitWarning(new RequestContentLengthMismatchError())
      }
    }

    socket.off('error', onDrain)
      .off('drain', onDrain)
    if (error) {
