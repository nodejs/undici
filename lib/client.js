...
class Client extends EventEmitter {
  constructor (url, {
    ...
    if (maxAbortedPayload != null && !Number.isFinite(maxAbortedPayload)) {
      throw new ConfigurationError('invalid maxAbortedPayload')
    }

    if (timeout != null && !Number.isFinite(timeout)) {
      throw new ConfigurationError('invalid timeout')
    }
  ...
  request (opts, callback) {
    ...
    if (typeof callback !== 'function') {
      throw new ConfigurationError('invalid callback')
    }

    if (!opts || typeof opts !== 'object') {
      process.nextTick(callback, new ConfigurationError('invalid opts'), null)
      return
    }
  ...
  pipeline (opts, handler) {
    if (typeof handler !== 'function') {
      return new PassThrough().destroy(new ConfigurationError('invalid handler'))
    }
    ...
    this.enqueue({ ...opts, body: req }, function (err, data, resume) {
      ...
      if (!body) {
        ret.destroy(new ConfigurationError('invalid body'))
        return
      }
  ...
  stream (opts, factory, callback) {
    ...
    if (typeof factory !== 'function') {
      process.nextTick(callback, new ConfigurationError('invalid factory'), null)
      return
    }
