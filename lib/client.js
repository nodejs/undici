...
function _resume (client, sync) {
  while (true) {
    ...
    if (client[kRunning] > 0 && util.isAsyncIterable(request.body)) {
...
function write (client, request) {
  const { body, method, path, host, upgrade, headers, contentType } = request
  ...
  if (!body) {
    ...
    writeBlob({ client, request, socket, contentLength, contentType, header, expectsPayload })
  } else if (util.isStream(body)) {
    writeStream({ client, request, socket, contentLength, contentType, header, expectsPayload })
  } else if (util.isIterable(body)) {
    writeIterable({ client, request, socket, contentLength, contentType, header, expectsPayload })
  } else {
    assert(false)
  }
...
async function writeBlob ({ client, request, socket, contentLength, contentType, header, expectsPayload }) {
  const { body } = request
  ...
  try {
    ...
    if (contentType == null && body.type) {
      socket.write(`content-type: ${body.type}\r\n\r\n`, 'ascii')
    } else {
      socket.write('\r\n', 'ascii')
    }
