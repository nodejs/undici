...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnHeadersComplete] (versionMajor, versionMinor, rawHeaders, method,
    ...
    if (upgrade) {
      util.destroy(socket, new NotSupportedError('upgrade supported'))
      return true
    }
  ...
  [HTTPParser.kOnBody] (chunk, offset, length) {
    ...
    const ret = !request.aborted
      ? request.onBody(chunk, offset, length)
      : null
  ...
  [HTTPParser.kOnMessageComplete] () {
    ...
    request.onComplete(request, headers)
...
function write (client, request) {
  ...
  const { body, signal, header, hostname } = request
