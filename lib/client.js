...
async function writeIterable (client, request, socket, contentLength, header, expectsPayload) {
  ...
  function onFinished (err) {
    ...
    if (!iteratorNormalCompletion && iterator && iterator.return) {
      iterator.return()
    }
  ...
  try {
    ...
    const isSync = !!body[Symbol.iterator]
    iterator = isSync ? body[Symbol.iterator]() : body[Symbol.asyncIterator]()
    while (true) {
      let nextResult
      try {
        ...
        // throw from next is completion
        iteratorNormalCompletion = true
        throw err
      }

      const { value: chunk, done } = nextResult
      if (done || finished || socket.destroyed) {
        if (done) {
          // done is normal
          iteratorNormalCompletion = true
        }
      ...
      if (!len) {
        if (isSync) {
          // await as otherwise this could run forever
...
function write (client, request) {
  ...
  if (!body) {
    ...
    assert(util.isStrictIterable(body), 'body should be iterable')
