...
function _connect (client) {
  ...
  client._parser = new HTTPParser(HTTPParser.RESPONSE)
  ...
  client._parser[HTTPParser.kOnHeaders] = () => {}
  client._parser[HTTPParser.kOnHeadersComplete] = ({ statusCode, headers }) => {
  ...
  client._parser[HTTPParser.kOnBody] = (chunk, offset, length) => {
    _read += length

    if (_body.destroyed) {
      if (_read > client[kMaxAbortedPaylot]) {
        socket.destroy()
      }
  ...
  client._parser[HTTPParser.kOnMessageComplete] = () => {
    onFinished()
  }
...
class Client extends EventEmitter {
  constructor (url, opts = {}) {
    ...
    this[kQueue] = Q((request, callback) => {
      ...
      const { host, method, path, body, chunked, rawHeaders } = request

      if (body && this[kQueue].running() > 1) {
        // Request with body are usually not idempotent.
        // Ensure that no other request is inflight and
        // could cause failure.

        // TODO: This is too strict. Would be better if
        // user could specificy which requests are idempotent
        // or not. Where if the users does not specify then
        // all requests that have body and/or method is POST.

        // Request with body can error while other requests
      ...
      if (body) {
        ...
        // explicitly reset the connection once reesponse has been
      ...
      this[kInflight].push({ request, callback, reset: Boolean(body) })
