...
function onParserTimeout (parser) {
  const { socket, timeoutType, timeoutValue, client } = parser
...
function _resume (client, sync) {
  while (true) {
    ...
    if (socket && connected && client[kRunning] === 0 && client[kPending] === 0) {
...
function write (client, request) {
  ...
  if (!body) {
    ...
    const onFinished = function (err) {
      ...
      assert(socket[kParser].timeoutType === TIMEOUT_HEADERS)
