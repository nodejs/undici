...
class Client extends EventEmitter {
  constructor (url, opts = {}) {
    ...
    const dispatch = (request, cb) => {
      if (this.destroyed) {
        return cb(new Error('The client is destroyed'))
      }

      if (this.socket.destroyed) {
        this[kQueue].unshift(request, cb)
        this[kQueue].pause()
        cb()
        return
      }
      ...
      this[kInflight].push({
        ...
        callback: request.wrap((err, result) => {
    ...
    this[kResetParser] = () => {
      ...
      this.parser[HTTPParser.kOnHeadersComplete] = ({ statusCode, headers }) => {
        ...
        if (!skipBody) {
          this[kLastBody] = new Readable({ read: this[kRead] })
