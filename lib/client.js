...
function resume (client, sync) {
  ...
  if (client[kMaxRequests] && client[kRunningIdx] >= client[kMaxRequests]) {
    util.destroy(client[kSocket], new InformationalError('max requests reached'))
    resetQueue(client)
  } else if (client[kRunningIdx] > 256) {
    resetQueue(client)
  }
}
