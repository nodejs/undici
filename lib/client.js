...
class Parser {
    ...
    this.inst = new WebAssembly.Instance(mod, {
      env: {
        ...
        wasm_on_header_field: (p, at, len) => {
          ...
          return this.onHeader(this.getBuffer(at, len)) || 0
        },
        ...
        wasm_on_body: (p, at, length) => {
          return this.onBody(this.getBuffer(at, length)) || 0
        },
  ...
  getBuffer (at, len) {
    assert(this.bufferRef)
    assert(at >= this.bufferPtr && len <= this.bufferRef.length)

    const start = at - this.bufferPtr
    const end = start + len

    return this.bufferRef.slice(start, end)
  }
