...
const kHeaderMaxSize = Symbol('kHeaderMaxSize')
...
const on_header = (p, at, len) => {
  this[kHeaderSize] += len
  if (this[kHeaderSize] >= this[kHeaderMaxSize]) {
    inst.exports.llhttp_set_error_reason(this[kPtr], 'HPE_HEADER_OVERFLOW:Header overflow')
    return constants.ERROR.USER
  }
  currentParser[kHeaders].push(Buffer.from(inst.exports.memory.buffer, at, len).toString())
  return 0
}

const wasm_on_header_field = (p, at, len) => {
  assert(currentParser[kHeaders].length % 2 === 0)
  return on_header(p, at, len)
}

const wasm_on_header_value = (p, at, len) => {
  assert(currentParser[kHeaders].length % 2 === 1)
  return on_header(p, at, len)
}
...
const wasm_on_headers_complete = p => {
  ...
  return currentParser[kOnHeadersComplete](rawHeaders, statusCode, upgrade, shouldKeepAlive)
}

const wasm_on_body = (p, at, length) => {
  currentParser[kOnBody](inst.exports.memory.buffer, at, length)
  return 0
}
...
class HTTPParser {
  constructor (maxHeadersSize = 8 * 1024, lenient = false) {
    ...
    this[kHeaders] = []
    this[kHeaderMaxSize] = maxHeadersSize
    this[kHeaderSize] = 0
  ...
  [kOnHeadersComplete] (rawHeaders, statusCode, upgrade, shouldKeepAlive) {
    return 0
  }
  ...
  [kOnMessageComplete] (rawTrailers) {
    return 0
  }
