...
class Cache {
  ...
  async matchAll (request = undefined, options = {}) {
    ...
    if (arguments.length === 0) {
      // 5.2.1
      responses.push(...this.#relevantRequestResponseList)
    } else { // 5.3
      // 5.3.1
      const requestResponses = this.#queryCache(r, options)

      // 5.3.2
      responses.push(...requestResponses)
    }
  ...
  async put (request, response) {
    ...
    if (innerResponse.headersList.contains('vary')) {
      // 7.1.
      const fieldValues = innerResponse.headersList.get('vary').split(',').map(h => h.trim()).filter(h => h.length)
