...
class SqliteCacheStore {
  ...
  constructor (opts) {
    ...
    this.#getReadableValuesQuery = this.#db.prepare(`
      SELECT
        id,
        body,
        statusCode,
    ...
    this.#lockValueQuery = this.#db.prepare(
      'UPDATE cacheInterceptor SET locked = true WHERE id = ?'
    )

    this.#updateBodyAndUnlockQuery = this.#db.prepare(
      'UPDATE cacheInterceptor SET body = ?, rawTrailers = ?, locked = false WHERE id = ?'
    )
  ...
  createReadStream (req) {
    ...
    const value = this.#findValue(req, this.#getReadableValuesQuery, true)

    return value ? new SqliteStoreReadableStream(value) : undefined
  }
  ...
  createWriteStream (req, opts) {
    ...
    const value = this.#findValue(req, this.#getValuesQuery, false)
    if (!value) {
      ...
      // Check if there's already another request writing to the value
      if (value.locked) {
        return undefined
      }

      this.#lockValueQuery.run(value.id)
    ...
    writable.on('close', () => {
      if (!errored) {
        this.#updateBodyAndUnlockQuery.run(
  ...
  deleteByOrigin (origin) {
    this.#deleteByOriginStatement.run(origin)
  }

  /**
   * @param {import('../../types/dispatcher.d.ts').default.RequestOptions} req
   * @param {*} query The sql query TODO update type when Node types get updated
   * @param {boolean} parseJsonColumns
   * @returns {SqliteStoreValue}
   */
  #findValue (req, query, parseJsonColumns) {
    /**
    ...
    for (const value of values) {
      if (now >= value.deleteAt) {
        // We reached the expired values, let's cleanup the table & return early
        this.#deleteExpiredValuesStatement.run(Date.now())
        return undefined
      }
    ...
    if (matchingRow.locked !== undefined) {
      value.locked = matchingRow.locked
    }
