...
class SqliteCacheStore {
  ...
  constructor (opts) {
    if (opts) {
      ...
      if (opts.maxDbEntries !== undefined) {
        if (
          typeof opts.maxDbEntries !== 'number' ||
          !Number.isInteger(opts.maxDbEntries) ||
          opts.maxDbEntries < 0
        ) {
          throw new TypeError('SqliteCacheStore options.maxDbEntries must be a non-negative integer')
        }
        this.#maxDbEntries = opts.maxDbEntries
      }
    }
    ...
    this.#getOldCacheAtQuery = this.#db.prepare(
      'SELECT cachedAt FROM cacheInterceptorV1 ORDER BY cachedAt ASC LIMIT 1 OFFSET ?'
    )

    this.#deleteOldCacheAtQuery = this.#db.prepare(
      'DELETE FROM cacheInterceptorV1 WHERE cachedAt <= ?'
    )
  }
  ...
  prune () {
    ...
    const oldestCachedAt = this.#getOldCacheAtQuery.get(total - 1)?.cachedAt

    const res = this.#deleteOldCacheAtQuery.run(oldestCachedAt)
    return res.changes
  }
