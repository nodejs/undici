...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnHeadersComplete] (versionMajor, versionMinor, rawHeaders, method,
    ...
    if (!request) {
      // TODO: This check will only work if pipelining is turned off. Otherwise,
      // there is no way to detect that the server sends multiple replies and any
      // further response would be corrupted.
      // TODO: Use proper error class.
      socket.destroy(new Error('Invalid server response'))
      return true
    }
