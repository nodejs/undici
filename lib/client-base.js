...
function resume (client) {
  while (true) {
    ...
    if (client[kWriting]) {
      if (util.bodyLength(client[kWriting]) !== 0) {
        return
      }

      // TODO: Avoid this hack
      client[kWriting].emit(kEnd)
...
function write (client, request) {
  ...
  if (!body) {
    ...
    if (!expectsPayload) {
      client[kReset] = true
      assert(client.running === 0, 'only schedule reset if no other request is running')
    }
    ...
    const onData = function (chunk) {
      ...
      if (bytesWritten === 0) {
        ...
        if (!expectsPayload) {
          client[kReset] = true
          assert(client.running === 1, 'only schedule reset if no other request is running')
        }
      }
    ...
    const onFinished = function (err) {
      ...
      body
        ...
        .removeListener(kEnd, onFinished)
        .removeListener('error', onFinished)
      ...
      if (!err) {
        ...
        // client.running can be 0 when server responds before request payload
        // has been fully sent.
