...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnHeadersComplete] (versionMajor, versionMinor, rawHeaders, method,
    ...
    request.onHeaders(statusCode, parseHeaders(rawHeaders, headers), resumeSocket)
...
function resume (client) {
  while (true) {
    ...
    const bodyLength = util.bodyLength(request.body)

    // TODO: Add test for pipelining w/ pipeline which has an empty body
    // stream for e.g. GET requests.
    if (client.running && util.isStream(request.body) && bodyLength !== 0) {
...
function write (client, request) {
  ...
  if (contentLength === 0) {
    if (method !== 'PUT' && method !== 'POST' && method !== 'PATCH') {
      ...
      // TODO: What other methods could cause problems?
    }
    ...
    assert(!request.reset, 'no body must not cause connection reset')
    assert(!request.body, 'empty content-length must not have body')
  } else {
    if (method === 'HEAD' || method === 'GET') {
      ...
      assert(request.reset, 'HEAD or GET with body must reset connection')
    } else {
      // TODO: What other methods could cause problems?
    }

    assert(request.body, 'content-length must have body')
  }

  assert(!request.reset || !client.running, 'do not reset connection while pipelining')
  ...
  if (!body) {
    ...
    const onData = function (chunk) {
      ...
      // TODO: If bytesWritten === contentLength ensure body does not intend to
      // send more data.
      if (contentLength !== undefined && bytesWritten > contentLength) {
        this.destroy(new ContentLengthMismatch())
      }
