...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnHeadersComplete] (versionMajor, versionMinor, rawHeaders, method,
    ...
    if (statusCode === 101) {
      ...
      util.destroy(socket, new NotSupportedError('101 response not supported'))
      return true
    }
...
function resume (client) {
  while (true) {
    ...
    if (request.reset && util.bodyLength(request.body) === 0) {
      // If body has ended and is empty cleanup request state.
      request.reset = false
    }
...
function write (client, request) {
  ...
  {
    const bodyLength = util.bodyLength(body, true)
  ...
  if (!body) {
    socket.write(CRLF)
    if (contentLength !== undefined && method !== 'HEAD') {
      assert(method !== 'GET' || request.reset)
    ...
    assert(method !== 'GET' || method !== 'HEAD' || request.reset)
