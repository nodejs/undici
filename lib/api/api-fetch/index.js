...
const { requestBadPort, responseLocationURL, requestCurrentURL } = require('./util')
...
function httpNetworkFetch (
  ...
  return new Promise((resolve) => context.dispatcher.dispatch({
    path: requestCurrentURL(request).pathname + requestCurrentURL(request).search,
    origin: requestCurrentURL(request).origin,
    ...
    onHeaders (status, headersList, resume, statusText) {
      ...
      response = new Response(stream, { status, statusText })[kState]
      response.headersList.push(...headers[kHeadersList])
