...
function abortFetch (p, request, responseObject) {
  ...
  if (request.body !== null) {
    // NOTE: We don't have any way to check if body is readable. However,
    // cancel will be noop if state is "closed" and reject if state is
    // "error", which means that calling cancel() and catch errors
    // has the same effect as checking for "readable".
  ...
  // body with error.
  if (response.body !== null && context.controller) {
...
function fetching ({
  ...
  if (request.origin === 'client') {
    ...
    request.origin = request.currentURL.origin
  }
...
async function mainFetch (fetchParams, recursive = false) {
  ...
  response = await (async () => {
    ...
    // NOTE: This branch is not possible since we cannot set request mode
    // to "navigate".
    ...
    if (!/^https?:/.test(request.currentURL.protocol)) {
  ...
  let internalResponse = response.status === 0
    ? response
...
function httpRedirectFetch (fetchParams, response) {
  ...
  // URL’s fragment.
  let locationURL = actualResponse.headersList.get('location')
  ...
  try {
    locationURL = new URL(locationURL, request.currentURL)
  } catch (err) {
  ...
  if (
    locationURL.origin !== request.currentURL.origin &&
    request.origin !== locationURL.origin
  ) {
  ...
  if (
    ...
    (actualResponse.status === 303 && !['GET', 'HEADER'].includes(request.method))
  ) {
    // then:
...
async function httpNetworkOrCacheFetch (
  ...
  if (response == null) {
    ...
    if (
      ...
      (forwardResponse.status >= 200 && forwardResponse.status)
    ) {
      // TODO
    }
...
function httpNetworkFetch (
  ...
  const context = this
  return new Promise((resolve) => context.dispatcher.dispatch({
    path: request.currentURL.pathname + request.currentURL.search,
    origin: request.currentURL.origin,
    ...
    onConnect (abort) {
      if (context.terminated) {
        ...
        context.abort = () => abort(new AbortError())
      }
    },
    ...
    onHeaders (statusCode, headers, resume) {
      ...
      // TODO: Not sure we should use new Response here.
      response = new Response(stream, {
        status: statusCode,
        // TODO: Get statusText from response?
