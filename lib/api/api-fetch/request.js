...
class Request {
    ...
    if ('referrer' in init) {
      ...
      if (!referrer === '') {
        ...
        try {
          ...
          const error = new TypeError('invalid referrer')
          error.cause = err
          throw error
        }
  ...
  clone () {
    ...
    const clonedRequest = makeRequest(this[kState])
    clonedRequest.body = cloneBody(clonedRequest.body)
...
function makeRequest (init) {
  ...
  const request = {
    ...
    urlList: init.urlList
      ? [...init.urlList.map(url => new URL(url))]
      : []
  }
  request.currentURL = request.urlList[request.urlList.length - 1]
