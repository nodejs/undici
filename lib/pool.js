...
const kClosedReject = Symbol('closed reject')
...
class Pool {
  constructor (url, {
    ...
    function onDrain () {
      ...
      if (pool[kClosedPromise] && queue.isEmpty()) {
        Promise
          .all(pool[kClients].map(c => c.close()))
          .then(pool[kClosedResolve], pool[kClosedReject])
      }
    }
    ...
    try {
      ...
      if (!this[kClosedPromise]) {
        if (this[kQueue].isEmpty()) {
          ...
          this[kClosedPromise] = new Promise((resolve, reject) => {
            this[kClosedResolve] = resolve
            this[kClosedReject] = reject
          })
        }
      }

      if (cb) {
        this[kClosedPromise].then(() => cb(null, null), (err) => cb(err, null))
      } else {
