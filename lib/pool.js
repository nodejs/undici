...
const { kTLSSession, kTLSSessionCache } = require('./core/symbols')
...
class Pool extends EventEmitter {
  constructor (origin, { connections, ...options } = {}) {
    ...
    this[kPending] = 0
    this[kConnected] = 0
    this[kTLSSessionCache] = new TLSSessionCache()
    // this.session = null
    ...
    this[kOnDrain] = function onDrain () {
      ...
      if (pool[kClosedResolve] && queue.isEmpty()) {
        Promise
          ...
          .then(() => pool[kTLSSessionCache].clear())
    ...
    this[kOnTLSSession] = function updateClientTLSSession (session) {
      pool[kTLSSessionCache].set(/* client */ this, session)
    }
  }
      ...
      if (!client) {
        if (!this[kConnections] || this[kClients].length < this[kConnections]) {
          ...
          if (!options.tls || (options.tls.reuseSessions !== false)) {
            const session = this[kTLSSessionCache].getSession()
            if (!options.tls) {
              options.tls = {
                session
              }
            } else {
              options.tls.session = session
            }
          }
          ...
          if (!options.tls || options.tls.reuseSessions !== false) {
            client.on(kTLSSession, this[kOnTLSSession])
          }
