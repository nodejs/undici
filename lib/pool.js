...
class Pool extends EventEmitter {
  constructor (origin, { connections, ...options } = {}) {
    ...
    this[kConnected] = 0
    // this.session = null
    // this.session = Buffer.from('fofofofo')
    ...
    this[kOnTLSSession] = function onTLSSession (session) {
      pool.cacheTLSSession(session)
    }
  }
  ...
  cacheTLSSession (session) {
    if (session) {
      console.log(`pool ${this.id} cache new session`, session.toString('hex').substr(-5))
      this[kTLSSession] = session
    }
  }

  getCachedTLSSession () {
    console.log(`pool ${this.id} fetch session`, this[kTLSSession] ? this[kTLSSession].toString('hex').substr(-5) : null)
    return this[kTLSSession]
  }
    ...
    try {
      ...
      if (!client) {
        if (!this[kConnections] || this[kClients].length < this[kConnections]) {
          ...
          if (!options.tls || (options.tls.reuseSessions !== false && this[kTLSSession])) {
            if (!options.tls) {
              options.tls = {
                session: this[kTLSSession]
              }
            } else {
              options.tls.session = this[kTLSSession]
            }
          }
          ...
          if (!options.tls || options.tls.reuseSessions !== false) {
            client.on('session', (session) => this.cacheTLSSession(session))
          }
