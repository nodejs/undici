...
class Request {
  ...
  onConnect (pause, resume) {
    ...
    try {
      if (this._onConnect) {
        this._onConnect.call(this.self, resume)
      }
    } catch (err) {
      this.onError(err)
    }
  ...
  onUpgrade (statusCode, headers, socket) {
    ...
    try {
      if (this._onUpgrade) {
        this._onUpgrade.call(this.self, statusCode, headers, socket)
      }
    } catch (err) {
      this.onError(err)
    }
  ...
  onHeaders (statusCode, headers) {
    ...
    try {
      if (statusCode < 200) {
        if (this._onInfo) {
          this._onInfo.call(this.self, statusCode, headers)
        }
      } else {
        if (this._onHeaders) {
          this._onHeaders.call(this.self, statusCode, headers)
        }
      }
    } catch (err) {
      this.onError(err)
  ...
  onBody (chunk, offset, length) {
    ...
    if (this._onData) {
      try {
        return this._onData.call(this.self, chunk.slice(offset, offset + length))
      } catch (err) {
        this.onError(err)
        return null
      }
  ...
  onComplete (trailers) {
    ...
    try {
      if (this._onComplete) {
        this._onComplete.call(this.self, trailers)
      }
    } catch (err) {
      this.onError(err)
    }
  ...
  onError (err) {
    ...
    if (this._onError) {
      this._onError.call(this.self, err)
    }
  }
}
