...
class Request extends AsyncResource {
  constructor (opts, callback) {
    ...
    if (!opts) {
      throw new ConfigurationError('no options passed')
    }

    const { path, method, body, headers, idempotent, opaque } = opts

    if (!(typeof path === 'string' && path[0] === '/')) {
      throw new ConfigurationError('path must be a valid path')
    }

    if (!(typeof method === 'string' && methods[method])) {
      throw new ConfigurationError('method must be a valid method')
    }

    if (!isValidBody(body)) {
      throw new ConfigurationError('body must be a string, a Buffer or a Readable stream')
    }
