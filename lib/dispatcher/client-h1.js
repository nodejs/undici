...
class Parser {
  ...
  // Socket-read idle timeout: a transport-level inactivity budget that, unlike
  // `setTimeout(TIMEOUT_BODY)`, is refreshed only when body bytes are actually
  // read from the socket (see onBody) and fires regardless of `paused`. This
  // detects a silent/half-open upstream even while the consumer is applying
  // backpressure (where the body timeout is intentionally suppressed).
  startIdleTimeout (delay) {
    if (this.idleTimeout) {
      timers.clearTimeout(this.idleTimeout)
      this.idleTimeout = null
    }
    if (delay) {
      this.idleTimeout = timers.setFastTimeout(onParserIdleTimeout, delay, this.timeoutWeakRef)
    }
  }

  refreshIdleTimeout () {
    if (this.idleTimeout?.refresh) {
  ...
  destroy () {
    ...
    this.idleTimeout && timers.clearTimeout(this.idleTimeout)
    this.idleTimeout = null
  ...
  onHeadersComplete (statusCode, upgrade, shouldKeepAlive) {
    ...
    if (this.statusCode >= 200) {
      ...
      const bodyIdleTimeout = request.bodyIdleTimeout != null
        ? request.bodyIdleTimeout
        : client[kBodyIdleTimeout]
      this.startIdleTimeout(bodyIdleTimeout)
    } else if (this.timeout) {
      if (this.timeout.refresh) {
        this.timeout.refresh()
      }
  ...
  onBody (buf) {
    ...
    // Body bytes were read from the socket: the upstream is alive, so reset the
    // socket-read idle budget. This is the only place it is refreshed — notably
    // NOT in resume(), which is driven by the consumer rather than the socket.
  ...
  onMessageComplete () {
    ...
    // Body fully received: stop the socket-read idle timer so it cannot fire on
    // the now-idle (possibly keep-alive reused) socket.
...
function onParserIdleTimeout (parserWeakRef) {
  const parser = parserWeakRef.deref()
  if (!parser) {
    return
  }

  // Unlike onParserTimeout's TIMEOUT_BODY branch, this fires regardless of
  // `paused`. A paused parser means the *consumer* asked us to stop reading,
  // but a socket that has produced no body bytes for `bodyIdleTimeout` ms is a
  // transport-level stall (e.g. a silent/half-open upstream) that must surface
  // as an error so the request can fail/retry instead of hanging forever.
  util.destroy(parser.socket, new BodyTimeoutError())
}
