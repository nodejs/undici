...
class Parser {
  ...
  onMessageComplete () {
    ...
    if (socket[kWriting]) {
      ...
      // Start idle-socket validation as soon as the socket becomes idle so
      // the event-loop yield (GHSA-35p6-xmwp-9g52) runs during body
      // consumption instead of after the next dispatch. Sequential
      // fetch().text() / request() can then reuse the socket without
      // paying a second timer.
      if (socket[kSocketUsed] && socket[kIdleSocketValidation] === 0) {
