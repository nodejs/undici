...
const { wellknownHeaderNamesByIndex } = require('../core/constants.js')
...
function lazyllhttp () {
  ...
  return new WebAssembly.Instance(mod, {
    env: {
       ...
       * @param {number} at
       * @param {number} len
       * @param {number} wellknown index into wellknownHeaderNamesByIndex, or 0
       * @returns {number}
       */
      wasm_on_header_field: (p, at, len, wellknown) => {
        assert(currentParser.ptr === p)
        const start = at - currentBufferPtr + currentBufferRef.byteOffset
        // The raw name is taken from the chunk, in the peer's case. The WASM
        // glue lowercased its own copy in linear memory, which gives the
        // parsed key: a preallocated string for a well-known name, decoded
        // otherwise.
        const offset = at - currentBufferPtr
        const key = wellknown !== 0
          ? wellknownHeaderNamesByIndex[wellknown]
          : currentBuffer.latin1Slice(offset, offset + len)
        return currentParser.onHeaderField(new FastBuffer(currentBufferRef.buffer, start, len), key)
      },
      /**
       * @param {number} p
       * @param {number} at
...
class Parser {
  ...
  constructor (client, socket, { exports }) {
    ...
    this.upgrade = false
    this.headers = []
    // The lowercased name of each field line in `headers`. Request consumes it
    // synchronously while parsing a field section, so one array serves every
    // section; entries past the current section are stale and never read.
    this.headerKeys = []
    this.headersSize = 0
  ...
  execute (chunk) {
    ...
    if (
      ...
      currentBuffer = new FastBuffer(llhttp.memory.buffer, currentBufferPtr, currentBufferSize)
    }
  ...
  /**
   * @param {Buffer} buf raw fragment of a header name
   * @param {string} key the same fragment, lowercased
   * @returns {number}
   */
  onHeaderField (buf, key) {
    const len = this.headers.length
    // The index of this field line, for a new name and a continued one alike.
    const field = len >> 1

    if ((len & 1) === 0) {
      this.headers.push(buf)
      this.headerKeys[field] = key
    } else {
  ...
  onHeaderValue (buf) {
    ...
    const headerName = this.headerKeys[(len >> 1) - 1]
  ...
  onUpgrade (head) {
    ...
    try {
      request.onRequestUpgrade(statusCode, headers, socket, statusText, this.headerKeys)
    } catch (err) {
  ...
  onHeadersComplete (statusCode, upgrade, shouldKeepAlive) {
    ...
    // Responses to HEAD and 204/304 responses never carry content
    if (request.method === 'HEAD' || statusCode === 204 || statusCode === 304) {
      // A non-zero Content-Length on a 204/304 means the server may also have
      // sent content inappropriately, which would then be parsed as the
      // next response on this connection. Accept it, but do not reuse the
      // connection. HEAD requests are fine tho.
      if (request.method !== 'HEAD' && this.contentLength > 0) {
        this.shouldKeepAlive = false
      }
      this.contentLength = -1
    }
    ...
    const pause = request.onResponseStart(statusCode, headers, this.resume, statusText, this.headerKeys) === false
  ...
  onMessageComplete () {
    ...
    request.onResponseEnd(headers, this.headerKeys)
