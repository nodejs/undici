  ...
  return new WebAssembly.Instance(mod, {
    env: {
      ...
      wasm_on_header_field: (p, at, len, wellknown) => {
        assert(currentParser.ptr === p)
        const start = at - currentBufferPtr + currentBufferRef.byteOffset
        return currentParser.onHeaderField(new FastBuffer(currentBufferRef.buffer, start, len), wellknown)
      },
      /**
       * @param {number} p
       * @param {number} at
...
class Parser {
  ...
  constructor (client, socket, { exports }) {
    ...
    this.upgrade = false
    this.headers = []
    // The well-known name index of each field line in `headers` (0 for other
    // names), as the WASM glue reported it. Request consumes it synchronously
    // while parsing a field section, so one array serves every section.
    this.headerWellknown = new Uint16Array(32)
    this.headersSize = 0
  ...
  onHeaderField (buf, wellknown) {
    ...
    if ((len & 1) === 0) {
      ...
      if (field === this.headerWellknown.length) {
        const grown = new Uint16Array(field * 2)
        grown.set(this.headerWellknown)
        this.headerWellknown = grown
      }
      this.headerWellknown[field] = wellknown
    } else {
      this.headers[len - 1] = Buffer.concat([this.headers[len - 1], buf])
      // A name split across reads was matched piecewise, if at all.
      this.headerWellknown[field] = 0
    }
  ...
  onHeaderValue (buf) {
    ...
    let headerName = wellknownHeaderNamesByIndex[this.headerWellknown[(len >> 1) - 1]]
    if (headerName === undefined && (key.length === 10 || key.length === 14)) {
      headerName = util.bufferToLowerCasedHeaderName(key)
    }
  ...
  onUpgrade (head) {
    ...
    try {
      request.onRequestUpgrade(statusCode, headers, socket, statusText, this.headerWellknown)
    } catch (err) {
  ...
  onHeadersComplete (statusCode, upgrade, shouldKeepAlive) {
    ...
    const pause = request.onResponseStart(statusCode, headers, this.resume, statusText, this.headerWellknown) === false
  ...
  onMessageComplete () {
    ...
    request.onResponseEnd(headers, this.headerWellknown)
