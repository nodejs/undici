...
const HTTP1_ONLY_SUFFIX = '#http1-only'
...
function getOriginKey (origin, allowH2) {
  // Keep a separate cache entry for HTTP/1-only dispatchers so they do not
  // share a pool with requests that may negotiate HTTP/2.
  return allowH2 === false ? `${origin}${HTTP1_ONLY_SUFFIX}` : origin
}
...
class Agent extends DispatcherBase {
  ...
  [kDispatch] (opts, handler) {
    ...
    const key = getOriginKey(origin, allowH2)
...
Agent.getOriginKey = getOriginKey
Agent.HTTP1_ONLY_SUFFIX = HTTP1_ONLY_SUFFIX
