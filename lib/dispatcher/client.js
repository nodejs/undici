...
// maxConcurrentStreams. When the context isn't attached yet but the URL
// is https + allowH2, we may still negotiate h2 through ALPN, so use the
// same ceiling speculatively. Once ALPN resolves the value either stays
// put (h2) or contracts down to `getPipelining` (h1) and the request
// queue drains accordingly. We don't speculate for plain http (no ALPN)
// nor when allowH2 is explicitly false.
function getMaxConcurrent (client) {
  if (client[kHTTPContext]?.version === 'h2' || mayNegotiateH2(client)) {
    return client[kMaxConcurrentStreams]
  }
  return getPipelining(client)
}

function mayNegotiateH2 (client) {
  return client[kHTTPContext] == null &&
    client[kAllowH2] &&
...
class Client extends DispatcherBase {
  ...
  constructor (url, {
    ...
    this[kAllowH2] = allowH2 == null ? true : allowH2
  ...
  get [kBusy] () {
    ...
    // to spin up an additional Client. For h1 that fan-out is correct — each
    // socket only handles one pipelined request at a time. For h2 we want
    // requests to multiplex onto a single session, so suppress that signal
    // both when an h2 context is attached *and* while we don't yet know the
    // protocol but ALPN may still negotiate h2 (https + allowH2): otherwise
    // the first concurrent burst would fork one Client per request during
    // the TLS handshake.
    const allowsMux = this[kHTTPContext]?.version === 'h2' || mayNegotiateH2(this)
