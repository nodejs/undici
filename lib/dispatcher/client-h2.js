...
function connectH2 (client, socket) {
  ...
  return {
    ...
    */
    busy (request) {
      if (request != null) {
        if ((request.upgrade != null || request.method === 'CONNECT') && session[kRemoteSettings] === false) {
          // If we are waiting for remote settings, we cannot accept new requests
          // Otherwise can lead to wrong assumptions from local settings
          return true
        }
      }
...
function writeH2 (client, request) {
  ...
  if (upgrade || method === 'CONNECT') {
    ...
    if (upgrade === 'websocket') {
      ...
      // TODO: how do we handle refused streams when websocket are not supported?
