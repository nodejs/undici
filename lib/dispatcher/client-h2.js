...
function onData (chunk) {
  ...
  if (request.onResponseData(chunk) === false) {
    pauseResponseStream(stream, state)
  }
}

// The body timeout measures how long the peer takes to send data. While the
// consumer applies backpressure the peer can't send (it's out of flow-control
// window), and Http2Stream's timer, which counts stream activity, would fire.
// Stop it while paused and start a fresh one on resume, like HTTP/1.1, which
// ignores the body timeout while its parser is paused.
function pauseResponseStream (stream, state) {
  stream.pause()
  if (state.bodyTimeout) {
    stream.setTimeout(0)
  }
}

// Passed to the handler as its resume callback, bound to the stream.
function resumeResponseStream () {
  const stream = this
  const state = stream[kRequestStreamState]

  if (state != null && state.bodyTimeout && stream.isPaused()) {
...
function responseRawHeaders (rawHeaders, headers) {
  // HTTP/2 exposes raw fields as strings; the dispatcher raw header contract
  // uses Buffers, as with HTTP/1. The first pair is the :status pseudo-header.
  return rawHeaders?.slice(2).map(value => Buffer.from(value, 'latin1')) ?? headers
}

function onInterimResponse (headers, _flags, rawHeaders) {
  const stream = this
  const state = stream[kRequestStreamState]
  ...
  request.onResponseStart(Number(statusCode), responseRawHeaders(rawHeaders, headers), noop, '')
}

function onResponse (headers, _flags, rawHeaders) {
  const stream = this
  const state = stream[kRequestStreamState]
  ...
  // Node's HTTP/2 header object discards repeated values for fields it treats
  // as single-valued. Use the raw field list so consumers see every value.
  // The :status pseudo-header is not a response header.
  if (request.onResponseStart(statusCode, responseRawHeaders(rawHeaders, headers), resumeResponseStream.bind(stream), '') === false) {
    pauseResponseStream(stream, state)
  }
