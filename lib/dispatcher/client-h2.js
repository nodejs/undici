...
function connectH2 (client, socket) {
  ...
  return {
    ...
    */
    busy (_request) {
      return session[kRemoteSettings] === false
    }
  }
}
  ...
  if (upgrade || method === 'CONNECT') {
    ...
    if (upgrade === 'websocket') {
      ...
      stream.once('response', (headers, _flags, rawHeaders) => {
        ...
        if (statusCode === 200 && stream.rstCode === 0) {
          // Handle 200 for websocket over h2 as per RFC-8441 is done
          // we forward the control to the user
      ...
      stream.on('error', (_err) => {
        if (stream.rstCode === 7 || stream.rstCode === 8) {
