...
// TODO: fix - if request attempting to upgrade connection and remote settings not received
// we need to halt execution until we receive the remote settings
// Other requests can continue as normal
...
function writeH2 (client, request) {
  ...
  // TODO: support CONNECT properly as per spec
  if (upgrade || method === 'CONNECT') {
    session.ref()

    // We cannot upgrade to websocket if extended CONNECT protocol is not supported
    if (upgrade === 'websocket') {
      // console.log('doing upgrade', upgrade, session.remoteSettings)
      // We halt execution until we receive the remote settings

      if (session[kRemoteSettings] === false) {
        console.log('waiting for remote settings')
        client[kPendingIdx] = client[kRunningIdx]
        assert(client[kRunning] === 0)
      ...
      console.log('remote settings received', session[kEnableConnectProtocol])
      ...
      headers[':protocol'] = upgrade

      stream = session.request(headers, { endStream: false, signal })

      stream.once('response', (headers, _flags, rawHeaders) => {
        const { [HTTP2_HEADER_STATUS]: statusCode } = headers

        request.onUpgrade(statusCode, rawHeaders, stream)
