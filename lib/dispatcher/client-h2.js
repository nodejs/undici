...
function onInterimResponse (headers, _flags, rawHeaders) {
  ...
  request.onResponseStart(Number(statusCode), headers, noop, '', rawHeaders)
}
...
function onResponse (headers, _flags, rawHeaders) {
  ...
  // as single-valued. Pass the raw fields without converting them to Buffers;
  // the controller materializes those only when requested.
  if (request.onResponseStart(statusCode, headers, resumeResponseStream.bind(stream), '', rawHeaders) === false) {
    pauseResponseStream(stream, state)
  }
