...
function onInterimResponse (headers, _flags, rawHeaders) {
  ...
  request.onResponseStart(Number(statusCode), rawHeaders?.slice(2) ?? headers, noop, '')
}
...
function onResponse (headers, _flags, rawHeaders) {
  ...
  if (request.onResponseStart(statusCode, rawHeaders?.slice(2) ?? headers, resumeResponseStream.bind(stream), '') === false) {
    pauseResponseStream(stream, state)
  }
