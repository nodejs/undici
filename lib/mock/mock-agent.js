...
class MockAgent extends Dispatcher {
  ...
  assertNoPendingInterceptors ({
    ...
    showUnusedInterceptors = false,
    showCallHistory = false,
    includeRequestDiff = false
  } = {}) {
    ...
    let message = pending.length === 1
      ? `1 interceptor is pending:\n\n${pendingInterceptorsFormatter.format(pending)}`
      : `${pending.length} interceptors are pending:\n\n${pendingInterceptorsFormatter.format(pending)}`

    // Add unused interceptors information
    if (showUnusedInterceptors) {
      const allInterceptors = []
      for (const [origin, result] of this[kClients].entries()) {
        const dispatches = result.dispatcher[kDispatches]
        allInterceptors.push(...dispatches.map(d => ({ ...d, origin })))
      }

      const unused = allInterceptors.filter(d => d.timesInvoked === 0)
      if (unused.length > 0) {
        message += `\n\n${unused.length} interceptors were never used:\n`
        message += pendingInterceptorsFormatter.format(unused)
      }
    }

    // Add call history if enabled and requested
    if (showCallHistory && this[kMockAgentIsCallHistoryEnabled]) {
      const callHistory = this.getCallHistory()
      if (callHistory) {
        const calls = callHistory.calls()
        message += `\n\nCall History (${calls.length} calls):`
        calls.slice(-5).forEach((call, index) => {
          message += `\n${index + 1}. ${call.method} ${call.fullUrl}`
        })
        if (calls.length > 5) {
          message += `\n... and ${calls.length - 5} more calls`
        }
      }
    }

    // Add request diff analysis if requested
    if (includeRequestDiff && this[kMockAgentIsCallHistoryEnabled]) {
      const callHistory = this.getCallHistory()
      if (callHistory) {
        const recentCalls = callHistory.calls().slice(-3)
        if (recentCalls.length > 0) {
          message += '\n\nRecent requests vs pending interceptors:'
          for (const call of recentCalls) {
            message += `\n\n${call.method} ${call.path}:`
            for (const pendingInterceptor of pending.slice(0, 2)) {
              const comparison = this.compareRequest(call, pendingInterceptor)
              if (comparison.differences.length > 0) {
                message += `\n  vs ${pendingInterceptor.method} ${pendingInterceptor.path}:`
                for (const diff of comparison.differences) {
                  message += `\n    ${diff.field}: expected "${diff.expected}", got "${diff.actual}" (similarity: ${diff.similarity})`
                }
              }
            }
          }
    ...
    throw new UndiciError(message.trim())
  }
