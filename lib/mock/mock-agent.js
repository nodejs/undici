...
class MockAgent extends Dispatcher {
  ...
  get (origin) {
    ...
    if (typeof originKey === 'string') {
      const http1OriginKey = Agent.getOriginKey(originKey, false)
      let http1Dispatcher = this[kClients].get(http1OriginKey)?.dispatcher

      if (!http1Dispatcher) {
        http1Dispatcher = this[kFactory](originKey, { ...this[kOptions], allowH2: false })
        this[kMockAgentSet](http1OriginKey, http1Dispatcher)
  ...
  dispatch (opts, handler) {
    ...
    const baseDispatcher = this.get(opts.origin)
    ...
    if (dispatchOpts.allowH2 === false && typeof dispatchOpts.origin === 'string') {
      const dispatcher = this[kClients].get(Agent.getOriginKey(dispatchOpts.origin, false))?.dispatcher
      return dispatcher.dispatch(dispatchOpts, handler)
    }

    return baseDispatcher.dispatch(dispatchOpts, handler)
  }
  ...
  pendingInterceptors () {
    ...
    return Array.from(mockAgentClients.entries())
      .filter(([origin]) => typeof origin !== 'string' || !origin.endsWith(Agent.HTTP1_ONLY_SUFFIX))
