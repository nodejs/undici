...
const kComposedAgent = Symbol('kComposedAgent')
...
class SnapshotAgent extends MockAgent {
  ...
  compose (...args) {
    // For recording/update modes, create a composed real agent with interceptors
    if (this[kSnapshotMode] === 'record' || this[kSnapshotMode] === 'update') {
      this[kComposedAgent] = this[kRealAgent].compose(...args)

      // Return a custom composed dispatcher that handles recording
      return this._createRecordingComposedDispatcher(...args)
    }

    return super.compose(...args)
  }

  _createRecordingComposedDispatcher (...args) {
    const composedRealAgent = this[kComposedAgent]
    const self = this
      ...
      get: (target, key) => {
        if (key === 'dispatch') {
          return function (opts, handler) {
            // Create a recording wrapper for the final response
            const responseData = {
              statusCode: null,
              headers: {},
              trailers: {},
            ...
            const recordingHandler = {
              ...
              onHeaders (statusCode, headers, resume, statusText) {
                // Only capture the final response (not redirects)
              ...
              onData (chunk) {
                // Only capture data for successful responses
              ...
              onComplete (trailers) {
                // Record only if we captured a successful response
                if (responseData.statusCode >= 200 && responseData.statusCode < 300) {
            ...
            // Use the composed real agent to handle the request with recording
            return composedRealAgent.dispatch(opts, recordingHandler)
          }
        }
        return target[key]
      }
  ...
  _recordAndReplay (opts, handler) {
    ...
    const recordingHandler = {
      ...
      onData (chunk) {
        responseData.body.push(chunk)
        const result = handler.onData?.(chunk)
        // Ensure we return a proper boolean - undefined should be treated as true
        return result !== false
      },
      onComplete (trailers) {
        responseData.trailers = trailers
        ...
        self[kSnapshotRecorder].record(opts, {
          ...
          // Log recording errors for debugging
          console.error('Recording error:', error)
        })
    ...
    const agent = this[kComposedAgent] || this[kRealAgent]
    agent.dispatch(opts, recordingHandler)
  }
  ...
  async close () {
    ...
    if (this[kComposedAgent]) {
      await this[kComposedAgent].close()
    } else if (this[kRealAgent]) {
