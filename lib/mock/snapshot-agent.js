    ...
    try {
        ...
        for (const [name, value] of Object.entries(response.headers)) {
          if (Array.isArray(value)) {
            for (const entry of value) {
              rawHeaders.push(Buffer.from(`${name}`), Buffer.from(`${entry}`))
            }
          } else {
      ...
      if (response.trailers) {
        for (const [name, value] of Object.entries(response.trailers)) {
          if (Array.isArray(value)) {
            for (const entry of value) {
              rawTrailers.push(Buffer.from(`${name}`), Buffer.from(`${entry}`))
            }
          } else {
            rawTrailers.push(Buffer.from(`${name}`), Buffer.from(`${value}`))
          }
        }
      }
