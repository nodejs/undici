...
class MockScope {
  ...
  registerCallHistory (name) {
    ...
    if (MockCallHistory[kMockCallHistoryGetByName](name) !== undefined) {
      throw new InvalidArgumentError(`a CallHistory with name ${name} already exist`)
    }

    // we want to be able to access history even if mockDispatch are deleted
    this[kMockCallHistory] = new MockCallHistory(name)
    this[kMockDispatch][kMockCallHistory] = this[kMockCallHistory]
