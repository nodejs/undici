...
const { consume } = require('../api/readable.js')
 ...
 */
async function mockDispatch (opts, handler) {
  // Get mock dispatch from built key
  const key = buildKey(opts)

  if (util.isAsyncIterable(key.body)) {
    const stream = util.ReadableStreamFrom(key.body)
    ...
    key.body = util.toUSVString(Buffer.concat(body))
  }
...
function buildMockDispatch () {
  ...
  return async function dispatch (opts, handler) {
    if (agent[kIsMockActive]) {
      try {
        await mockDispatch.call(this, opts, handler)
      } catch (error) {
