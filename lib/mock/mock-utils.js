...
function dispatchMockReply (mockDispatches, mockDispatch, key, opts, handler, resolvedResponse) {
  ...
  function handleReply (mockDispatches, _response = response) {
    ...
    if (isPromise(body)) {
      ...
      // A rejection is surfaced as a response error so the dispatch settles
      // instead of hanging, mirroring the reply options callback path.
      return body.then(
        (newData) => handleReply(mockDispatches, { ..._response, data: newData }),
        (err) => {
          // Don't emit if the request was aborted while awaiting the body
          if (aborted) {
            return
          }
          deleteMockDispatch(mockDispatches, key)
