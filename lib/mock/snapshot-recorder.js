 ...
 */
function formatRequestKey (opts, matchOptions = {}) {
  ...
  return {
    ...
    headers: filterHeadersForMatching(normalized, matchOptions),
 ...
 */
function filterHeadersForMatching (headers, matchOptions = {}) {
  ...
  const filtered = {}
  const ignoreSet = new Set(ignoreHeaders.map(h => caseSensitive ? h : h.toLowerCase()))
  ...
  for (const [key, value] of Object.entries(headers)) {
    ...
    if (matchHeaders && Array.isArray(matchHeaders)) {
      const matchSet = new Set(matchHeaders.map(h => caseSensitive ? h : h.toLowerCase()))
...
class SnapshotRecorder {
  constructor (options = {}) {
    ...
    process.emitWarning(
      'SnapshotRecorder is experimental and subject to change',
      'ExperimentalWarning'
    )
  ...
  async record (requestOpts, response) {
    ...
    const request = formatRequestKey(requestOpts, this.matchOptions)
  ...
  findSnapshot (requestOpts) {
    ...
    if (snapshot.responses && Array.isArray(snapshot.responses)) {
      const responseIndex = Math.min(snapshot.callCount, snapshot.responses.length - 1)
      snapshot.callCount = (snapshot.callCount || 0) + 1
...
module.exports = { SnapshotRecorder, formatRequestKey, createRequestHash, filterHeadersForMatching, filterHeadersForStorage, isUrlExcluded }
