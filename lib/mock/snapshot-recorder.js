 ...
 * @param {HeaderSets} cachedSets - Cached sets for ignore, exclude, and match headers
 ...
 *
 * @param {SnapshotRecorderMatchOptions} matchOptions - Matching options for headers
 * @returns {HeaderSets} - Cached sets for ignore, exclude, and match headers
 */
function createHeaderSetsCache (matchOptions = {}) {
  ...
  return {
    ignoreSet: new Set(ignoreHeaders.map(header => caseSensitive ? header : header.toLowerCase())),
    excludeSet: new Set(excludeHeaders.map(header => caseSensitive ? header : header.toLowerCase())),
    matchSet: new Set(matchHeaders.map(header => caseSensitive ? header : header.toLowerCase()))
  }
}
...
/**
 * @typedef {Object} HeaderSets
 * @property {Set<string>} ignoreSet - Set of headers to ignore for matching
 * @property {Set<string>} excludeSet - Set of headers to exclude from matching
 * @property {Set<string>} matchSet - Set of headers to match (empty means match
 */
...
class SnapshotRecorder {
  ...
  constructor (options = {}) {
    ...
    this.#headerSetsCache = createHeaderSetsCache(this.matchOptions)
  ...
  async record (requestOpts, response) {
    ...
    const request = formatRequestKey(requestOpts, this.#headerSetsCache, this.matchOptions)
    ...
    const responseData = {
      ...
      headers: filterHeadersForStorage(normalizedHeaders, this.#headerSetsCache, this.matchOptions),
