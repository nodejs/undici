 ...
 */
const validSRIHashAlgorithmTokenSet = ['sha256', 'sha384', 'sha512']
...
try {
  ...
  if (cryptoHashes.length === 0) {
    validSRIHashAlgorithmTokenSet.length = 0
  }

  for (let i = 0; i < validSRIHashAlgorithmTokenSet.length; i++) {
    const algorithm = validSRIHashAlgorithmTokenSet[i]
    // If the algorithm is not supported, remove it from the list.
    if (cryptoHashes.includes(algorithm) === false) {
      validSRIHashAlgorithmTokenSet.splice(i, 1)
      i--
    }
  }
  /* c8 ignore next 4 */
 ...
 */
function isValidSRIHashAlgorithm (algorithm) {
  return validSRIHashAlgorithmTokenSet.includes(algorithm)
}
...
function getStrongestMetadata (metadataList) {
  ...
  for (const item of metadataList) {
    ...
    if (newAlgorithmIndex > currentAlgorithmIndex) {
      ...
      // 3. Otherwise, newAlgorithmIndex and currentAlgorithmIndex are the same
...
function parseMetadata (metadata) {
  ...
  for (const item of metadata.split(' ')) {
    ...
    // 6. If algorithm-and-value[1] exists, set base64-value to
...
const applyAlgorithmToBytes = crypto.hash
  ...
  : /** @type {ApplyAlgorithmToBytes} */ (algorithm, bytes) => {
      return crypto.createHash(algorithm).update(bytes).digest('base64')
    }
