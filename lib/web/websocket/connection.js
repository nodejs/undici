...
function establishWebSocketConnection (url, protocols, client, handler, options) {
  ...
  const controller = fetching({
    ...
    processResponse (response) {
      function failHandshake (code, reason, cause) {
      ...
      if (response.type === 'error' || response.status !== 101) {
        ...
        if (response.socket?.session == null) {
          failHandshake(1002, 'Received network error or non-101 status code.', response.error)
          return
        }

        // HTTP2
        if (response.status !== 200) {
          failHandshake(1002, 'Received network error or non-200 status code.', response.error)
          return
        }
      }
      ...
      if (protocols.length !== 0 && !response.headersList.get('Sec-WebSocket-Protocol')) {
        failHandshake(1002, 'Server did not respond with sent protocols.')
        return
      }
      ...
      if (response.socket.session == null && response.headersList.get('Upgrade')?.toLowerCase() !== 'websocket') {
        failHandshake(1002, 'Server did not set Upgrade header to "websocket".')
        return
      }
      ...
      if (response.socket.session == null && response.headersList.get('Connection')?.toLowerCase() !== 'upgrade') {
        failHandshake(1002, 'Server did not set Connection header to "upgrade".')
        return
      }
      ...
      if (response.socket.session == null) {
        ...
        if (secWSAccept !== digest) {
          failHandshake(1002, 'Incorrect hash received in Sec-WebSocket-Accept header.')
          return
        }
      }
      ...
      if (secExtension !== null) {
        ...
        if (!extensions.has('permessage-deflate')) {
          failHandshake(1002, 'Sec-WebSocket-Extensions header does not match.')
          return
        }
      }
      ...
      if (secProtocol !== null) {
        ...
        if (requestProtocols === null || !requestProtocols.includes(secProtocol)) {
          failHandshake(1002, 'Protocol was not set in the opening handshake.')
          return
        }
      }
