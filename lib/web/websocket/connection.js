...
function establishWebSocketConnection (url, protocols, client, handler, options) {
  ...
  const supportsPerMessageDeflate = handler.agent.webSocketOptions?.perMessageDeflate
  const permessageDeflate = supportsPerMessageDeflate
  ...
  const controller = fetching({
    ...
    processResponse (response) {
      ...
      if (secExtension !== null) {
        const negotiatedExtensions = parseExtensions(permessageDeflate)
        extensions = parseExtensions(secExtension)

        if (extensions.has('permessage-deflate') !== negotiatedExtensions.has('permessage-deflate')) {
...
function failWebsocketConnection (handler, code, reason, cause) {
  ...
  if (isConnecting(handler.readyState)) {
    ...
    handler.onSocketClose(reason != null ? new TypeError(reason, cause ? { cause } : undefined) : undefined)
