...
const { kEnumerableProperty, markPromiseAsHandled } = require('../../../core/util')
...
class WebSocketStream {
  ...
  constructor (url, options = undefined) {
    ...
    // Both promises can reject without any user action (an abort signal, or
    // an unclean close), and many users never observe one of them. Mark them
    // as handled so that this isn't an unhandled rejection, which crashes
    // Node.js by default. This matches how the Streams Standard treats the
    // closed promise of stream readers and writers.
    markPromiseAsHandled(this.#openedPromise.promise)
    markPromiseAsHandled(this.#closedPromise.promise)
