...
class WebSocketStream {
  ...
  #write (chunk) {
    ...
    if (isView || isArrayBuffer(chunk)) {
      if (isView) {
        if (!isArrayBuffer(chunk.buffer)) {
          // See /websockets/stream/tentative/write.any.html
          promise.reject(new TypeError('cannot write a TypedArray with a shared buffer'))
          return promise.promise
        }
      } else if (chunk.resizable) {
        // See /websockets/stream/tentative/write.any.html
        promise.reject(new TypeError('cannot write a resizable ArrayBuffer'))
        return promise.promise
      }
