...
function multipartFormDataParser (input, mimeType) {
  ...
  while (true) {
    ...
    if (position.position === input.length - 2 && input.at(-1) === 0x2d && input.at(-2) === 0x2d) {
      return entryList
    }
  ...
  while (true) {
    ...
    switch (new TextDecoder().decode(headerName).toLowerCase()) {
      case 'content-disposition': {
        ...
        if (input.indexOf(formDataNameBuffer, position.position) !== position.position) {
          return 'failure'
        }
...
function removeChars (str, leading, trailing, predicate) {
  ...
  if (leading) {
    while (lead < str.length && predicate(str[lead])) lead++
  }

  if (trailing) {
    while (trail > 0 && predicate(str[trail])) trail--
  }

  return lead === 0 && trail === str.length - 1 ? str : str.subarray(lead, trail + 1)
}
