...
const { FormData, setFormDataState, getFormDataBoundary, setFormDataBoundary } = require('./formdata')
...
function extractBody (object, keepalive = false) {
  ...
  if (typeof object === 'string') {
    ...
    let boundary = getFormDataBoundary(object)
    if (boundary === null) {
      boundary = `----formdata-undici-0${`${random(1e11)}`.padStart(11, '0')}`
      setFormDataBoundary(object, boundary)
    }
