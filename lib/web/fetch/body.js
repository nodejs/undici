 ...
 *
 * @param {import('../../../types').BodyInit} bodyInit - The BodyInit object to extract from
 ...
 */
function extractBody (bodyInit, keepalive = false) {
  // 1. Let stream be null.
  let stream = null

  // 2. If object is a ReadableStream object, then set stream to object.
  if (webidl.is.ReadableStream(bodyInit)) {
    stream = bodyInit
  } else if (webidl.is.Blob(bodyInit)) {
    // 3. Otherwise, if object is a Blob object, set stream to the
    //    result of running object’s get stream.
    stream = bodyInit.stream()
  } else {
  ...
  if (typeof bodyInit === 'string') {
  ...
  } else if (webidl.is.URLSearchParams(bodyInit)) {
    // URLSearchParams
    ...
    source = new Uint8Array(bodyInit.slice())
  } else if (ArrayBuffer.isView(bodyInit)) {
    // BufferSource/ArrayBufferView

    // Set source to a copy of the bytes held by object.
    source = new Uint8Array(bodyInit.buffer.slice(bodyInit.byteOffset, bodyInit.byteOffset + bodyInit.byteLength))
  } else if (webidl.is.FormData(bodyInit)) {
    const boundary = generateMultipartBoundaryId()
    ...
    for (const [name, value] of bodyInit) {
  ...
  } else if (typeof bodyInit[Symbol.asyncIterator] === 'function') {
    ...
    if (util.isDisturbed(bodyInit) || bodyInit.locked) {
      throw new TypeError(
    ...
    stream =
      webidl.is.ReadableStream(bodyInit) ? bodyInit : ReadableStreamFrom(bodyInit)
  }
  ...
  if (action != null) {
    ...
    stream = new ReadableStream({
      async start () {
        iterator = action(bodyInit)[Symbol.asyncIterator]()
      },
