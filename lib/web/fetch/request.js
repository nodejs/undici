...
/**
 * Fetch's "If init is not empty" check is based on user-specified
 * RequestInit members, not dictionary defaults. undici's `priority`
 * member defaults to `"auto"`, so `Object.keys(convertedInit)` is
 * never empty and would otherwise force the expensive header
 * clone/clear/re-append path on every `fetch(url)` / `new Request(url)`.
 *
 * Matches the spec / Servo / Chromium "any members present" test.
 *
 * @param {object | null | undefined} init
 * @returns {boolean}
 */
function requestInitHasUserMembers (init) {
  return init != null && (
...
class Request {
  ...
  constructor (input, init = undefined) {
    ...
    const initHasKey = requestInitHasUserMembers(init)
