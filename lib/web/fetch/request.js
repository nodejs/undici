...
class Request {
  ...
  constructor (input, init = undefined) {
    ...
    if (typeof input === 'string') {
      ...
      // Note: Step 12 copies this request into a new one and makeRequest fills
      // every missing field with its default, so the URL list is all it needs.
      request = { urlList: [parsedURL] }
    ...
    // Note: Without init["headers"], these steps empty the header list and
    // append a copy of it back, which leaves it unchanged, so they are skipped.
    // init["headers"] is never a Headers object here: the HeadersInit
    // converter turns one into a list of its entries, which fill() appends.
    if (initHasKey && init.headers !== undefined) {
      ...
      getHeadersList(this.#headers).clear()
