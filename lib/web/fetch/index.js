...
function appendHeadersListFromResponseHeaders (headersList, headers, rawHeaders, start = 0) {
  if (Array.isArray(rawHeaders)) {
    for (let i = start; i < rawHeaders.length; i += 2) {
      const name = rawHeaders[i]
      const nameStr = typeof name === 'string' ? name.toLowerCase() : bufferToLowerCasedHeaderName(name)
      ...
      return new Promise((resolve, reject) => agent.dispatch(
        ...
        },
        {
          ...
          onResponseStart (controller, status, headers, statusText) {
            ...
            const rawHeadersH2 = controller?.rawHeadersH2
            const rawHeaders = rawHeadersH2 ?? controller?.rawHeaders ?? []
            const headersList = new HeadersList()
            appendHeadersListFromResponseHeaders(headersList, headers, rawHeaders, rawHeadersH2 == null ? 0 : 2)
