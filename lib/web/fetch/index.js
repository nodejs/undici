...
// publishIfContext will check if there is context if so,
// it will add error in the case of an error before publishing
// the event if not it will return right away
function publishIfContext (name, context, error) {
  if (!context) return
  if (error) context.error = error
  channels[name].publish(context)
}

// handlePromise will handle both fulfilled/rejected promises
// from p and will publish events accordingly
...
// checking if we have subscribers at the beginning of the fetch call
// this will set context if there are subscribers to the start channel
function subscribersCheck (input, init) {
  const hasSubscribers = channels.start.hasSubscribers ||
...
function fetch (input, init = {}) {
  const context = subscribersCheck(input, init)
  ...
  try {
    publishIfContext('start', context)

    try {
      requestObject = new Request(input, init)
    } catch (e) {
      p.reject(e)
      publishIfContext('fetchError', context, e)
    ...
    const processResponse = (response) => {
      ...
      try {
        ...
        if (response.type === 'error') {
          publishIfContext('fetchError', context, response.error)
        ...
        publishIfContext('asyncStart', context)
        publishIfContext('asyncEnd', context)
