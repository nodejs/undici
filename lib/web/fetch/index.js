...
const { addAbortListener, bufferToLowerCasedHeaderName, isBlobLike } = require('../../core/util')
const { kReplayableBody } = require('../../core/symbols')
...
async function httpNetworkFetch (
  ...
  if (request.body == null && fetchParams.processRequestEndOfBody) {
    ...
    // processBodyError, and fetchParams’s task destination.
    const createRequestBody = (stream) => (async function * () {
      try {
    ...
    // Recreate only sources whose serialized bytes stay stable across
    // extraction. FormData is intentionally excluded because extracting it
    // generates a multipart boundary while the request keeps its existing
    // Content-Type header.
    if (typeof source === 'string' || source instanceof Uint8Array || isBlobLike(source)) {
      requestBody = {
        [kReplayableBody]: true,
        [Symbol.asyncIterator] () {
          return createRequestBody(safelyExtractBody(source)[0].stream)
        }
      }
    } else {
      requestBody = createRequestBody(request.body.stream)
    }
  }
