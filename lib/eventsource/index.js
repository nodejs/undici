...
const { mainFetch, Fetch } = require('../fetch')
...
const { MessageEvent, ErrorEvent } = require('../websocket/events')
...
class EventSource extends EventTarget {
  ...
  async #connect () {
    ...
    fetchParam.processResponse = (response) => {
      ...
      if (mimeType === 'failure' || mimeType.essence !== 'text/event-stream') {
        this.close()
        this.dispatchEvent(new ErrorEvent('error', { message: 'Invalid content-type' }))
        return
      }
  ...
  async #reconnect () {
    ...
    this.dispatchEvent(new ErrorEvent('error', { message: 'Reconnecting' }))

    // 2. Wait a delay equal to the reconnection time of the event source.
    await setTimeout(this.#settings.reconnectionTime)
    ...
    if (this.#settings.lastEventId !== '') {
      this.#request.headers.set('last-event-id', this.#settings.lastEventId, true)
    }
