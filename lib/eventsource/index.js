...
const { fetching, finalizeAndReportTiming } = require('../fetch')
...
class EventSource extends EventTarget {
  ...
  constructor (url, eventSourceInitDict = {}) {
    ...
    this.#request = new Request(this.#url, initRequest)

    this.#request.headersList = initRequest.headers
    this.#request.urlList = this.#request.urlList || [new URL(this.#url)]
  ...
  #connect () {
    ...
    // 12. Set request's initiator type to "other".
    options.processResponseEndOfBody = (response) =>
      finalizeAndReportTiming(response, 'other')

    if (this.#lastEventId) {
      options.request.headers.set('last-event-id', this.#lastEventId, true)
    }
    ...
    options.processResponseEndOfBody = processEventSourceEndOfBody
