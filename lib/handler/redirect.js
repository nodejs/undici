...
class RedirectHandler {
  constructor (agent, { maxRedirections, history, ...opts }, handler) {
    this.agent = agent
    this.location = null
    this.abort = null
    this.opts = opts
  ...
  onConnect (abort, context) {
    this.history = context && Array.isArray(context.history)
      ? context.history
      : []
  ...
  onHeaders (statusCode, headers, resume) {
    this.location = this.maxRedirections === 0 ? null : parseLocation(statusCode, headers)
    ...
    this.opts.maxRedirections = this.maxRedirections - 1
