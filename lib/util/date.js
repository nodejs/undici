...
// The rfc850-date window slides with the clock, so the parser needs the
// current year. Reading it as `new Date().getUTCFullYear()` on every parse
// allocates in a path the cache handler walks per response, so the year is
// cached and revalidated against the precomputed start of the next one.
let cachedYear = new Date().getUTCFullYear()
let cachedYearEndsAt = Date.UTC(cachedYear + 1, 0, 1)

/**
 * The current UTC year: a `Date.now()` compare in the common case, and the
 * full read once a year.
 *
 * @returns {number}
 */
function currentUtcYear () {
  const now = Date.now()
  if (now >= cachedYearEndsAt) {
    cachedYear = new Date(now).getUTCFullYear()
    cachedYearEndsAt = Date.UTC(cachedYear + 1, 0, 1)
  }
  return cachedYear
}
