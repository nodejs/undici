...
class FormDataParser extends Writable {
  ...
  constructor (opts) {
    ...
    this.#opts.fileHwm = opts?.fileHwm
  ...
  run (callback) {
    while (true) {
      if (this.#state === states.INITIAL) {
        ...
        if (
          (bytes[0] !== chars['-'] || bytes[1] !== chars['-']) &&
          (bytes[0] !== chars.cr || bytes[1] !== chars.lf)
        ) {
          this.destroy(new Error('FormData should start with -- or CRLF'))
          return
        }
        ...
        if (!this.#boundary.equals(bytes)) {
          ...
          return
        } else if (this.#info.limits.parts >= this.#opts.limits.parts) {
          this.emit('partsLimit')
          this.#info.skipPart = 'partsLimit'
          this.#state = states.READ_BODY
          continue
        }

        this.#info.limits.parts++
        ...
        if (bufferLength === this.#boundary.length) {
          ...
          if (
            ...
            if (this.#info.limits.fields >= this.#opts.limits.fields) {
              this.#info.skipPart = 'fieldsLimit'
              this.emit('fieldsLimit')

              this.#state = states.BOUNDARY

              // Remove all body chunks for the current field
              this.#info.body.chunks.length = 0
              this.#info.body.length = 0

              // Add the rest of the bytes back to the buffer to parse
