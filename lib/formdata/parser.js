      ...
      if (this.#state === states.INITIAL) {
        ...
        if (bufferLength === this.#boundary.length) {
          ...
          if (this.#info.stream !== null) {
            ...
            const fullBody = new TextDecoder('utf-8', { fatal: true }).decode(
              Buffer.concat(body.chunks, body.length)
            )
            ...
            this.emit(
              ...
              fullBody,
              {
                nameTruncated: false, // todo
                valueTruncated: false, // todo
    ...
    while (position.position < buffer.length) {
      ...
      if (name !== 'content-disposition') {
        ...
        while (position.position < buffer.length) {
          ...
          const attribute = collected.toString('ascii')
          const equalIdx = attribute.indexOf('=')
          const name = attribute.slice(0, equalIdx).trim()
          const value = attribute[equalIdx + 1] === '"'
            ? collectAnHTTPQuotedString(attribute.slice(equalIdx + 1), { position: 0 }, true)
            : attribute.slice(equalIdx + 1)
