...
class Agent extends EventEmitter {
  ...
  get(origin) {
    ...
    if (!pool) {
      pool =
        self[kAgentOpts] && self[kAgentOpts].connections === 1
...
function dispatchFromAgent(requestType) {
  return (
    url,
    { agent = globalAgent, method = 'GET', ...opts } = {},
    ...additionalArgs
  ) => {
    if (opts.path != null) {
    ...
    if (client && typeof client[requestType] !== 'function') {
      throw new InvalidReturnValueError(
        `Client returned from Agent.get() does not implement method ${requestType}`
      )
    }
