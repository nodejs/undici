...
const WeakRef = global.WeakRef || class CompatWeakRef {
  constructor (value) {
    this.value = value
  }
...
const FinalizationRegistry = global.FinalizationRegistry || class CompatFinalizer {
...
class Agent extends Dispatcher {
  constructor ({ factory = defaultFactory, maxRedirections = 0, ...options } = {}) {
    ...
    this[kOnDrain] = ({ url, targets }) => {
      agent.emit('drain', { url, targets: [agent, ...targets] })
    }

    this[kOnConnect] = ({ url, targets }) => {
      agent.emit('connect', { url, targets: [agent, ...targets] })
    }

    this[kOnDisconnect] = ({ url, targets, err }) => {
      agent.emit('disconnect', { url, targets: [agent, ...targets], err })
    }
  }
