...
const { getGlobalDNSResolver } = require('./global-dns-resolver')
...
function getDNSResolver (options) {
  ...
  if (options?.dnsResolver?.disable) {
    return undefined
  }
  ...
  // without any options fallback to the global one
  return getGlobalDNSResolver()
}
