...
const kSorted = Symbol('headers map sorted')
...
class HeadersList {
  ...
  sortMap () {
    this[kHeadersMap] = new Map([...this[kHeadersMap]].sort((a, b) => a[0] < b[0] ? -1 : 1))
    this[kSorted] = true
  }
  ...
  * [Symbol.iterator] () {
    ...
    for (const [key, val] of this[kHeadersMap].entries()) {
      yield [key, val]
    }
  }
}
...
class Headers {
  ...
  _getHeadersListIterable () {
    ...
    return new HeadersList(this[kHeadersList])
  }

  * keys () {
    const listFreezed = this._getHeadersListIterable()
    for (const key of listFreezed[kHeadersMap].keys()) {
      yield key
    }
  }
  ...
  * values () {
    const listFreezed = this._getHeadersListIterable()
    for (const val of listFreezed[kHeadersMap].values()) {
      yield val
    }
  }
  ...
  * entries () {
    const listFreezed = this._getHeadersListIterable()
    for (const [key, val] of listFreezed[kHeadersMap]) {
      yield [key, val]
    }
  }
  ...
  forEach (...args) {
    ...
    const listFreezed = this._getHeadersListIterable()
    listFreezed[kHeadersMap].forEach((value, index) => {
