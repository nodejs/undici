...
class Response {
  ...
  static json (data, init = {}) {
    ...
    // ensure that data can be serialized
    // JSON.stringify(unserializable) will return undefined in some cases.
    toUSVString(data)

    // 1. Let responseObject be the result of creating a Response object,
    ...
    // 2. Perform initialize a response given responseObject, and init.
    initializeResponse(responseObject, init)

    // 3. If responseObject’s response’s status is a null body status,
    //    then throw a TypeError.
    if ([101, 204, 205, 304].includes(init.status)) {
      throw new TypeError(`init["status"] cannot be ${init.status}.`)
    }

    // 4. Set responseObject’s response’s body to the result of running
    //    serialize a JavaScript value to JSON bytes on data.
    responseObject[kState].body = new TextEncoder('utf-8').encode(JSON.stringify(data))

    // 5. If responseObject’s response’s header list does not contain `Content-Type`,
    //    then append (`Content-Type`, `application/json;charset=utf-8`) to
    //    responseObject’s response’s header list.
    if (!responseObject[kState].headersList.has('content-type')) {
      responseObject[kState].headersList.append(
        'content-type',
        'application/json;charset=utf-8'
      )
    }

    // 6. Return responseObject.
    return responseObject
  }
...
function initializeResponse (response, init) {
  // 1. If init["status"] is not in the range 200 to 599, inclusive, then
  //    throw a RangeError.
  ...
  //    then throw a TypeError.
  if (
    init.statusText != null &&
    /[^\t|\x20|\x00-\x7F|\x80-\xFF|]/g.test(init.statusText)
  ) {
    throw new TypeError(
