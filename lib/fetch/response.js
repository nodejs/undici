...
class Response {
  ...
  static json (data, init = {}) {
    ...
    // 2. If init["status"] is a null body status, then throw a TypeError.
    // https://whatpr.org/fetch/1392.html#null-body-status
    if ([101, 204, 205, 304].includes(init.status)) {
      throw new TypeError(`init["status"] cannot be ${init.status}.`)
    }

    // 3. If init["statusText"] does not match the reason-phrase
    //    token production, then throw a TypeError.
    // eslint-disable-next-line
    if (init.statusText != null && !/^[\t|\x20|\x00-\x7F|\x80-\xFF]{1,}$/.test(init.statusText)) {
      throw new TypeError(
        'Failed to execute \'json\' on \'Response\': invalid statusText was provided.'
      )
    }

    // 4. Let responseObject be the result of creating a Response object,
    //    given a new response, "response", and this’s relevant Realm.
    const relevantRealm = { settingsObject: {} }
    const responseObject = new Response()
    responseObject[kRealm] = relevantRealm
    ...
    // 5. Set responseObject’s response’s status to init["status"].
    responseObject[kState].status = init.status

    // 6. Set responseObject’s response’s status message to init["statusText"].
    responseObject[kState].statusText = init.statusText

    // 7. If init["headers"] exists, then fill responseObject’s headers
    //    with init["headers"].
    if ('headers' in init && init.headers != null) {
      fill(responseObject[kState].headersList, init.headers)
    }

    // 8. Set responseObject’s response’s body to the result of running serialize
    //    a JavaScript value to a JSON bytes on data.
    responseObject[kState].body = extractBody(JSON.stringify(data))[0]

    // 9. If responseObject’s response’s header list does not contain
    //    `Content-Type`, then append (`Content-Type`, `application/json;charset=utf-8`)
    //    to responseObject’s response’s header list.
    if (!responseObject[kState].headersList.has('content-type')) {
      responseObject[kState].headersList.append('content-type', 'application/json;charset=utf-8')
    }
