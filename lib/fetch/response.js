...
function filterResponse (response, type) {
  ...
  if (type === 'basic') {
    ...
    return makeFilteredResponse(response, {
      type: 'basic',
      headersList: new HeadersList(...headers) // TODO (fix): Make proxy or freeze in dev mode?
    })
  } else if (type === 'cors') {
    ...
    const headers = []
    for (let n = 0; n < response.headersList.length; n += 2) {
      if (!corsSafeListedResponseHeaderNames.includes(response.headersList[n])) {
        headers.push(response.headersList[n + 0], response.headersList[n + 1])
      }
    }
    ...
    return makeFilteredResponse(response, {
      type: 'opaque',
      urlList: [], // TODO (fix): Make proxy or freeze in dev mode?
      status: 0,
      statusText: '',
      body: null
    })
    ...
    return makeFilteredResponse(response, {
      ...
      status: 0,
      statusText: '',
      headersList: new HeadersList(), // TODO (fix): Make proxy or freeze in dev mode?
      body: null
    })
  } else {
    assert(false)
