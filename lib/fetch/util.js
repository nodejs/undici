...
function responseLocationURL (response, requestFragment) {
  ...
  let location = response.headersList.lowerCaseGet('location')
...
function setRequestReferrerPolicyOnRedirect (request, actualResponse) {
  ...
  // 4. Return policy.
  const policyHeader = (headersList.lowerCaseGet('referrer-policy') ?? '').split(',')
...
function appendFetchMetadata (httpRequest) {
  ...
  httpRequest.headersList.lowerCaseSet('sec-fetch-mode', header)
...
function appendRequestOriginHeader (request) {
  ...
  if (request.responseTainting === 'cors' || request.mode === 'websocket') {
    if (serializedOrigin) {
      request.headersList.lowerCaseAppend('origin', serializedOrigin)
    }
