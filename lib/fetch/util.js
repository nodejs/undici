...
function serializeOrigin (origin) {
  ...
  let result = origin.scheme

  // 3. Append "://" to result.
  result += '://'

  // 4. Append origin's host, serialized, to result.
  result += serializeHost(origin.host)
...
function serializeRequestOrigin (request) {
  // If request’s tainted origin flag is set, then return "null".
  if (request.taintedOrigin) {
    return null
  }

  // Return request’s origin, serialized.
  return serializeOrigin(request.origin)
}
...
function isomorphicEncode(input) {
  ...
  for (const code of input) {
    if (code > 0x00ff) {
      throw new Error('Isomorphic encode failed')
    }
  }
...
function appendRequestOriginHeader (request) {
  ...
  let serializedOrigin = serializeRequestOrigin(request)
  if (serializedOrigin) {
    serializedOrigin = isomorphicEncode(serializedOrigin)
  }
