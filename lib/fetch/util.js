...
function corsCheck (request, response) {
  // 1.  Get `Access-Control-Allow-Origin` from response’s header list.
  const allowedOrigin = response.headersList.get('access-control-allow-origin')
  ...
  // 6. If `Access-Control-Allow-Credentials`  is `true`, then return success.
  const accessAllowedCredentials = response.headersList.get('access-control-allow-credentials')
  if (accessAllowedCredentials === 'true') {
    return 'success'
  }
