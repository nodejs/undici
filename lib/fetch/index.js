...
class Fetch extends EE {
  ...
  get cancelled () {
    return this.state === 'aborted' || this.state === 'terminated'
  }
}
...
function fetching ({
  ...
  mainFetch.call(fetchParams.controller, fetchParams)
    .catch(err => {
      fetchParams.controller.terminate({ reason: err })
    })
...
async function fetchFinale (fetchParams, response) {
  ...
  if (fetchParams.processResponseConsumeBody != null) {
    ...
    const processBody = (nullOrBytes) => processResponseConsumeBody(response, nullOrBytes)

    // 2. Let processBodyError be this step: run fetchParams’s process
    // response consume body given response and failure.
    processBodyError = (failure) => fetchParams.processResponseConsumeBody(response, failure)
    ...
    if (response.body == null) {
      ...
      try {
        for await (const bytes of response.body.stream) {
          processBody(bytes)
        }
      } catch (err) {
        processBodyError(err)
...
async function httpNetworkFetch (
  ...
  const context = fetchParams.controller
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      ...
      return makeNetworkError(this.terminated.reason)
    }
  ...
  context.on('terminated', onAborted)
  context.resume = async () => {
    ...
    while (true) {
      ...
      let bytes
      try {
        const { done, value } = await context.next()
        bytes = done ? undefined : value
      } catch (err) {
        if (context.ended && !timingInfo.encodedBodySize) {
      ...
      // into stream.
      context.controller.enqueue(new Uint8Array(bytes))
      ...
      // the ongoing fetch.
      if (!context.controller.desiredSize) {
        return
      }
    }
  }
  ...
  async function dispatch ({ body }) {
    ...
    return new Promise((resolve, reject) => context.dispatcher.dispatch(
      ...
      },
      {
        ...
        onComplete () {
          if (this.abort) {
            context.off('terminated', this.abort)
          }
