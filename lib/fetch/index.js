...
const gcTimeout = global.gc ? setTimeout(global.gc, 1) : null
function queueGC () {
  gcTimeout?.refresh()
}
    ...
    context.dispatcher.dispatch(
      ...
      },
      {
        ...
        onData (chunk) {
          ...
          // 4. See pullAlgorithm...
          const ret = this.decoder.write(bytes)
          if (!ret) {
            // The fetch specication allows users to skip consuming the response body
            // by relying on garbage collection to release connection resources.
            // Node has less agressive garbage collection so we try and force it
            // if/when the dispatcher is busy. This does not take pipelining into
            // consideration. This will only work if Node was started with the
            // --expose-gc flag. If/When landing in core we could get around this
            // specific limitation.
            // TODO: Should we further throttle this somehow? Maybe some form of
            // stall detection?
            queueGC()
          }
          return ret
