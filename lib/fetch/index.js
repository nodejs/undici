...
async function fetch (...args) {
  if (args.length < 1) {
    throw new TypeError(
      `Failed to execute 'fetch': 1 argument required, but only ${args.length} present.`
    )
  }
  if (
    args.length >= 1 &&
    ...
    throw new TypeError(
      "Failed to execute 'fetch': cannot convert to dictionary."
    )
  }
    ...
    response = await (async () => {
      ...
      if (request.redirect !== 'follow') {
        return makeNetworkError(
          'redirect mode cannot be "follow" for this request'
        )
      }
