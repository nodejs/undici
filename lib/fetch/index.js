...
async function httpNetworkOrCacheFetch (
  ...
  if (
    ...
    httpRequest.headersList.set('cache-control', 'max-age=0')
  }
  ...
  if (httpRequest.cache === 'no-store' || httpRequest.cache === 'reload') {
    ...
    if (!httpRequest.headersList.has('pragma')) {
      httpRequest.headersList.set('pragma', 'no-cache')
    }

    // 2. If httpRequest’s header list does not contain `Cache-Control`,
    // then append `Cache-Control`/`no-cache` to httpRequest’s header list.
    if (!httpRequest.headersList.has('cache-control')) {
      httpRequest.headersList.set('cache-control', 'no-cache')
    }
  }
  ...
  if (httpRequest.headersList.has('range')) {
    httpRequest.headersList.set('accept-encoding', 'identity')
  }
  ...
  if (!httpRequest.headersList.has('accept-encoding')) {
    if (/^https:/.test(requestCurrentURL(httpRequest).protocol)) {
      httpRequest.headersList.set('accept-encoding', 'br, gzip, deflate')
    } else {
      httpRequest.headersList.set('accept-encoding', 'gzip, deflate')
    }
  }
