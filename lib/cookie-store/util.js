...
function cookieRetrieval (cookieStore) {
  ...
  cookieList.sort((a, b) => {
    ...
    // TODO: add creation time and then sort by that
    return a
  })
...
function queryCookies (cookieStore, name) {
  ...
  for (const cookie of cookieList) {
    ...
    if (typeof name === 'string') {
      ...
      // TODO: chrome doesn't decode this?
      const cookieName = cookie.cookieName
...
function createCookieListItem (cookie) {
  ...
  const expires = cookie.Expires ? cookie.Expires : null
...
function deleteCookie (name, cookieStore) {
  ...
  if (idx !== -1) {
    const deleted = cookieStore.splice(idx, 1)
