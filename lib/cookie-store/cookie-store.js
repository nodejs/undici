...
class CookieStore extends EventTarget {
   ...
   */
  get (name = {}) {
    if (this == null || !(#cookieStore in this)) {
      throw new TypeError('Illegal invocation')
  ...
  set (name, value = undefined) {
    ...
    const current = this.#cookieStore.find(
      c => c.cookieName === (name.name || name)
    )
    const currentIdx = current ? this.#cookieStore.indexOf(current) : -1
    ...
    if (typeof name === 'object') {
      ...
      if (currentIdx !== -1) {
        this.#cookieStore.splice(currentIdx, 1, r)
      } else {
        this.#cookieStore.push(r)
      }
