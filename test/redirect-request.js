...
//   (server, opts) => new undici.Pool(`http://${server}`, opts).compose(undici.interceptors.redirect(opts)),
//   (server, opts) => new undici.Client(`http://${server}`, opts).compose(undici.interceptors.redirect(opts))
// ]) {
