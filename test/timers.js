...
describe('timers', () => {
  test('timers exports a clearTimeout', (t) => {
    ...
    t.assert.ok(typeof timers.clearTimeout === 'function')
  })

  test('timers exports a setTimeout', (t) => {
    t.plan(1)

    t.assert.ok(typeof timers.setTimeout === 'function')
  })

  test('setTimeout instantiates a native NodeJS.Timeout when delay is lower or equal 1e3 ms', (t) => {
    t.plan(2)

    t.assert.strictEqual(timers.setTimeout(() => { }, 999)[timers.kFastTimer], undefined)
    t.assert.strictEqual(timers.setTimeout(() => { }, 1e3)[timers.kFastTimer], undefined)
  })
  ...
  test('setTimeout instantiates a FastTimer when delay is bigger than 1e3 ms', (t) => {
    ...
    t.assert.strictEqual(timeout[timers.kFastTimer], true)
  })

  test('clearTimeout can clear a node native Timeout', (t) => {
    t.plan(1)

    const nativeTimeoutId = setTimeout(() => { t.assert.fail() }, 1)
    t.assert.ok(timers.clearTimeout(nativeTimeoutId) === undefined)
    tick(10)
  })
  ...
  test('a FastTimer will get a _idleStart value after short time', async (t) => {
    ...
    t.assert.strictEqual(timer[timers.kFastTimer], true)
    t.assert.strictEqual(timer._idleStart, -1)

    tick(1e3)
    t.assert.notStrictEqual(timer._idleStart, -1)
  ...
  test('refresh correctly with timeout < TICK_MS', async (t) => {
    ...
    const timeout = timers.setTimeout(() => {
      ...
      t.assert.ok(delta >= -1, 'refreshed timer fired early')
      t.assert.ok(delta < ACCEPTABLE_DELTA, 'refreshed timer fired late')
    }, 80)
    ...
    setTimeout(() => t.assert.ok(true), 260)
  ...
  test('refresh correctly with timeout > TICK_MS', async (t) => {
    ...
    setTimeout(() => t.assert.ok(true), 1800)
  ...
  test('refresh correctly FastTimer with timeout > TICK_MS', async (t) => {
    ...
    const timeout = timers.setFastTimeout(() => {
      ...
      t.assert.ok(delta >= -1, `refreshed timer fired early (${delta} ms)`)
      t.assert.ok(delta < ACCEPTABLE_DELTA, `refreshed timer fired late (${delta} ms)`)
    }, 1001)
    ...
    setTimeout(() => t.assert.ok(true), 500)
  ...
  test('a FastTimer will only increment by the defined TICK_MS value', async (t) => {
    ...
    tick(250)
    t.assert.strictEqual(timers.now() - startInternalClock, 0)
    tick(250)
    t.assert.strictEqual(timers.now() - startInternalClock, 499)
    tick(250)
    t.assert.strictEqual(timers.now() - startInternalClock, 499)
    tick(250)
    t.assert.strictEqual(timers.now() - startInternalClock, 998)
    tick(250)
    t.assert.strictEqual(timers.now() - startInternalClock, 998)
    tick(250)
    t.assert.strictEqual(timers.now() - startInternalClock, 1497)
  ...
  test('meet acceptable resolution time', async (t) => {
    ...
    for (const target of testTimeouts) {
      timers.setTimeout(() => {
        ...
        t.assert.ok(delta >= -1, `${target}ms fired early`)
        t.assert.ok(delta < ACCEPTABLE_DELTA, `${target}ms fired late, got difference of ${delta}ms`)
      }, target)
    }
