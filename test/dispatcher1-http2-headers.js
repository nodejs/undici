...
for (const allowH2 of [false, true]) {
  test(`legacy headers and trailers are raw arrays (allowH2: ${allowH2})`, async t => {
    ...
    const dispatcher = new Dispatcher1Wrapper(new Agent({ allowH2, connect: { rejectUnauthorized: false } }))
    t.after(async () => {
    ...
    const result = await new Promise((resolve, reject) => {
      let headers
      const chunks = []
      dispatcher.dispatch({ origin: `https://localhost:${server.address().port}`, path: '/', method: 'GET' }, {
        onConnect () {},
...
test('legacy HTTP/2 CONNECT upgrade headers are raw arrays', async t => {
  ...
  const dispatcher = new Dispatcher1Wrapper(new Agent({ connect: { rejectUnauthorized: false } }))
  t.after(async () => {
    await dispatcher.destroy()
    await new Promise(resolve => server.close(resolve))
  })
  ...
  const headers = await new Promise((resolve, reject) => {
    dispatcher.dispatch({ origin: `https://localhost:${server.address().port}`, path: '/', method: 'CONNECT' }, {
      onConnect () {},
