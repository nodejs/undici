...
test('use proxy-agent to connect through proxy', async (t) => {
  ...
  const {
    ...
    headers,
    body
  } = await dispatcher.request({ path: '/', method: 'GET', origin: serverUrl })
...
test('use proxy agent to connect through proxy using Pool', async (t) => {
  ...
  const client = new Client(serverUrl)
  const dispatcher = client.compose(proxyInterceptor({ auth: Buffer.from('user:pass').toString('base64'), uri: proxyUrl, clientFactory }))
  const firstRequest = dispatcher.request({ path: '/', method: 'GET', origin: serverUrl })
  const secondRequest = await dispatcher.request({ path: '/', method: 'GET', origin: serverUrl })
...
test('use proxy-agent to connect through proxy using path with params', async (t) => {
  ...
  const {
    ...
    headers,
    body
  } = await dispatcher.request({ origin: serverUrl, method: 'GET', path: '/hello?foo=bar' })
  const json = await body.json()
...
test('use proxy-agent with auth', async (t) => {
  ...
  const client = new Client(serverUrl)
  const parsedOrigin = new URL(serverUrl)
  const dispatcher = client.compose(proxyInterceptor({
...
test('use proxy-agent with custom headers', async (t) => {
  ...
  await dispatcher.request({ origin: serverUrl, method: 'GET', path: '/hello?foo=bar', headers: { 'user-agent': 'BarBaz/1.0.0' } })
